...
- AN01 mHC HYPER-CONNECTIONS (research/055-architecture-matrix-7hop.md -- DeepSeek V3.2/V4 residual, arXiv 2512.24880): the engine's Round-3 `wubu_mhc.c` (sigmoid-constrained non-negative mixing, identity oracle) EXISTS + is wired (make test_mhc PASSES) -- the matrix audit's "0 files" claim was WRONG, corrected. The manifold-constrained softmax-row variant is a follow-up. `wired` (wubu_mhc, test_mhc PASSES)
