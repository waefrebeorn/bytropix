...
llm_graph_result * llama_context::process_ubatch(const llama_ubatch & ubatch, llm_graph_type gtype, llama_memory_context_i * mctx, ggml_status & ret) {
    ...
    // DUMP_LAYER_DIR: dump per-layer hidden states to files for cross-reference
    // Must deep-copy immediately because graph reuse recycles tensor buffers.
    {
        const char * dump_dir = getenv("DUMP_LAYER_DIR");
        if (dump_dir && dump_dir[0] && !res->t_layer_h.empty()) {
            mkdir(dump_dir, 0755);
            for (size_t i = 0; i < res->t_layer_h.size(); i++) {
                ggml_tensor * t = res->t_layer_h[i];
                if (!t || !t->data) continue;

                // deep-copy the data into a separate buffer
                size_t n_bytes = ggml_nelements(t) * ggml_type_size(t->type);
                void * copy = malloc(n_bytes);
                if (!copy) continue;

                // use ggml_backend_tensor_get for potentially non-CPU tensors
                // but for CPU tensors, direct memcpy works
                // we use memcpy since all tensors are on CPU in this setup
                memcpy(copy, t->data, n_bytes);
                ...
                if (t->type == GGML_TYPE_F32) {
                    ...
                    f32_buf = (float *)malloc(n_elems * sizeof(float));
                    if (f32_buf) {
                        for (size_t j = 0; j < n_elems; j++) {
                            f32_buf[j] = ggml_fp16_to_fp32(((ggml_fp16_t *)copy)[j]);
                        }
                    }
                    free(copy);
                } else {
                    // unknown type, just write raw
                    f32_buf = (float *)copy;
                    n_bytes = n_elems * sizeof(float); // wrong, but best effort
                }

                if (f32_buf) {
                    char fname[256];
                    snprintf(fname, sizeof(fname), "%s/ref_layer_%zu.bin", dump_dir, i);
                    FILE * fp = fopen(fname, "wb");
                    if (fp) {
                        fwrite(f32_buf, sizeof(float), n_elems, fp);
                        fclose(fp);
                    }
                    if (t->type != GGML_TYPE_F32) {
                        free(f32_buf);
        ...
        // Use for 1:1 parity verification against bytropix inference engine
        {
            ...
            if (int_dir && int_dir[0] && !res->t_intermediates.empty()) {
                mkdir(int_dir, 0755);
                for (size_t i = 0; i < res->t_intermediates.size(); i++) {
                    const auto & entry = res->t_intermediates[i];
                    ggml_tensor * t = entry.t;
                    if (!t || !t->data) continue;

                    size_t n_elems = ggml_nelements(t);
                    size_t n_bytes = n_elems * ggml_type_size(t->type);

                    char fname[512];
                    if (entry.il >= 0) {
                        snprintf(fname, sizeof(fname), "%s/L%d_%s.bin", int_dir, entry.il, entry.name.c_str());
                    } else {
                        snprintf(fname, sizeof(fname), "%s/global_%s.bin", int_dir, entry.name.c_str());
                    }
                    ...
                    if (t->type == GGML_TYPE_F32) {
                        ...
                        for (size_t j = 0; j < n_elems; j++) {
                            buf[j] = ggml_fp16_to_fp32(((ggml_fp16_t *)t->data)[j]);
                        }
                    } else {
                        free(buf);
                        continue;
                    ...
                    FILE * fp = fopen(fname, "wb");
                    if (fp) {
                        fwrite(buf, sizeof(float), n_elems, fp);
                        fclose(fp);
                    }
                    free(buf);
                }
