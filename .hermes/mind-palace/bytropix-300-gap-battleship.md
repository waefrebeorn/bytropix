...
| 103 | tools/train_real.c | Forward-only benchmark. `wubu_model_backward_from_embd` exists but train_real.c doesn't call it. GPU link error blocks build. | 🟡 Now fixed: CPU-only Makefile target (train_real_cpu) + forward-with-save loop + CE loss backward through output projection. Full training step: forward+save 0.36s, CE bwd 2.14s, model bwd 7.43s. All 8192 grad elements non-zero through 40 layers. GQA dequant-on-demand also fixed. |
