...
int main(void)
{
    ...
    wubu_buf_t b;
    if (wubu_buf_alloc(&b, 512) != 0) { printf("  buf alloc FAIL\n"); return 1; }
    ...
    long p0 = wubu_parameter_count(&m);
    int r3 = wubu_grow_stack_block(&m, 0);
    if (!r3 || m.n_layers != 5) { printf("  stack FAIL\n"); ok = 0; }
    long p1 = wubu_parameter_count(&m);
    long blk = p1 - p0;
    if (blk <= 0) { printf("  stack param growth FAIL\n"); ok = 0; }
    if (wubu_forward(&m, &b, toks, SEQ) != 0) { printf("  grown forward FAIL\n"); ok = 0; }
    ...
    {
        wubu_bp_t bp;
        if (wubu_bp_alloc(&bp, 512) != 0) { printf("  bp alloc FAIL\n"); return 1; }
        wubu_train_t tr;
        if (wubu_train_init(&tr, &m) != 0) { printf("  train init FAIL\n"); return 1; }
        m.n_layers = 2;
        float loss_small = wubu_bp_forward(&m, &b, &bp, toks, SEQ);
        ...
        float loss_grown = wubu_bp_forward(&m, &b, &bp, toks, SEQ);
        ...
        wubu_bp_backward(&m, &b, &bp, &tr, toks, SEQ);
        ...
        float epsf = 1e-2f;
        w[bi] = save + epsf;
        float l1 = wubu_bp_forward(&m, &b, &bp, toks, SEQ);
        w[bi] = save - epsf;
        float l2 = wubu_bp_forward(&m, &b, &bp, toks, SEQ);
        w[bi] = save;
