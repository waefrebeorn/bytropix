...
int main(void) {
    ...
    // Forward using F32 output proj (force F32 path)
    float *logits = (float *)malloc(vs * sizeof(float));
    wubu_model_forward_from_embd(&mdl, x, 1, 1, logits);
    
    // The forward already used quantized. Let me also dump the post-40-layer hidden state
    // before output projection to see if the issue is in output proj or layers.
