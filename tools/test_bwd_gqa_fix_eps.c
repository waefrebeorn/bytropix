// test_bwd_gqa_fix_eps.c — Verify FD passes with larger eps on HD=256 config
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
int main(void) {
    ...
    printf("=== FD eps sweep (HD=256, random values) ===\n");
    for (int ep = 2; ep <= 6; ep++) {
        float eps = powf(10.0f, -(float)ep);
        int q_ok = 0, k_ok = 0, v_ok = 0, NCHK = 40;
        ...
        printf("eps=1e-%d: dQ %d/%d  dK %d/%d  dV %d/%d\n", ep, q_ok, NCHK, k_ok, NCHK, v_ok, NCHK);
    }
