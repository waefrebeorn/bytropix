/*
 * test_barun_save.c -- the checkpoint round-trip test.
...
int main(int argc, char **argv)
{
    ...
    printf("=== test_barun_save (checkpoint export round-trip) ===\n");
    barun_model_t m, m2;
    CHECK(barun_load(&m, path) == 0, "load");
    CHECK(barun_save_safetensors(&m, "/tmp/barun_roundtrip.safetensors") == 0,
          "save as safetensors");
    CHECK(barun_load(&m2, "/tmp/barun_roundtrip.safetensors") == 0, "reload");
    ...
    for (int i = 0; i < BARUN_LAYERS; i++) {
        const barun_block_t *x = &m.blocks[i];
        const barun_block_t *y = &m2.blocks[i];
    ...
    CHECK(barun_parameter_count(&m2) == n, "param count preserved");
