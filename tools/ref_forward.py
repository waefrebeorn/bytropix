...
def main():
    ...
    embed_norm = W["model.embedding_norm.weight"].float()     # [d]
    d = embed.shape[1]
    ...
    hidden = rmsnorm(embed[ids], embed_norm)  # [T, d]
    ...
    logits = embed @ rmsnorm(hidden[-1:], embed_norm).T  # tied lm_head via embed_norm
    logits = logits[0]
