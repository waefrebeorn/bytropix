...
class ImageEncoder(nn.Module):
    ...
    def __call__(self, images: jnp.ndarray) -> jnp.ndarray:
        x=nn.Conv(32,(4,4),(2,2),name="conv1",dtype=self.dtype)(images);x=nn.gelu(x);x=nn.LayerNorm(epsilon=1e-5,dtype=jnp.float32)(x)
        x=nn.Conv(64,(4,4),(2,2),name="conv2",dtype=self.dtype)(x);x=nn.gelu(x);x=nn.LayerNorm(epsilon=1e-5,dtype=jnp.float32)(x)
        x=nn.Conv(128,(3,3),padding='SAME',name="conv3",dtype=self.dtype)(x);x=nn.gelu(x);x=nn.LayerNorm(epsilon=1e-5,dtype=jnp.float32)(x)
        x=jnp.mean(x,axis=(1,2));z=nn.Dense(self.d_model,name="proj_out",dtype=self.dtype)(x);return z
class TextEncoder(nn.Module):
    d_model: int; dtype: Any = DTYPE
    @nn.compact
    def __call__(self, text_embeds: jnp.ndarray) -> jnp.ndarray:
        h=nn.Dense(self.d_model*2,name="dense1",dtype=self.dtype)(text_embeds);h=nn.gelu(h);h=nn.LayerNorm(epsilon=1e-5,dtype=jnp.float32)(h)
        z=nn.Dense(self.d_model,name="dense2",dtype=self.dtype)(h);return z
class QuaternionComponentGenerator(nn.Module):
    d_model: int; dtype: Any = DTYPE
    @nn.compact
    def __call__(self, z: jnp.ndarray) -> jnp.ndarray:
        h=nn.Dense(self.d_model//2,name="dense1",dtype=self.dtype)(z);h=nn.gelu(h);scalar_out=nn.Dense(1,name="dense_out",dtype=self.dtype)(h)
...
class TopologicalObserver(nn.Module):
    ...
    def __call__(self, path_params_grid: jnp.ndarray) -> jnp.ndarray:
        ...
        delta_c,chi_c,radius=path_params[...,0],path_params[...,1],path_params[...,2];theta=jnp.linspace(0,2*jnp.pi,self.num_path_steps,dtype=self.dtype)
        delta_path=delta_c[...,None]+radius[...,None]*jnp.cos(theta);chi_path=chi_c[...,None]+radius[...,None]*jnp.sin(theta)
        t_co_steps=PoincareSphere.calculate_co_polarized_transmittance(delta_path,chi_path);accumulated_t_co=jnp.cumprod(t_co_steps,axis=-1)[:,:,-1]
        complex_measurement=jnp.stack([accumulated_t_co.real,accumulated_t_co.imag],axis=-1);return nn.Dense(self.d_model,name="patch_projector",dtype=self.dtype)(complex_measurement)
class TopologicalGenerativeDecoder(nn.Module):
    d_model: int; dtype: Any = DTYPE
    @nn.compact
    def __call__(self, z: jnp.ndarray) -> jnp.ndarray:
        h=nn.Dense(512,name="modulator_dense1",dtype=self.dtype)(z);h=nn.gelu(h);h=nn.Dense(16*16*32,name="modulator_dense2",dtype=self.dtype)(h)
        h=nn.gelu(h);h=h.reshape(h.shape[0],16,16,32);path_params_map=nn.Conv(3,(1,1),name="path_params_conv",dtype=self.dtype)(h)
        delta_c=nn.tanh(path_params_map[...,0])*jnp.pi;chi_c=nn.tanh(path_params_map[...,1])*(jnp.pi/4.0);radius=nn.sigmoid(path_params_map[...,2])*(jnp.pi/2.0)
        path_params_grid=jnp.stack([delta_c,chi_c,radius],axis=-1);return TopologicalObserver(d_model=self.d_model,dtype=self.dtype,name="observer")(path_params_grid)
class ImagePatchDecoder(nn.Module):
    ...
    def __call__(self,x):
        ...
        for i in range(2):
            B_up,H_up,W_up,C_up=h.shape;h=jax.image.resize(h,(B_up,H_up*2,W_up*2,C_up),'nearest');out_ch=max(ch//2,12)
...
class ChimeraAE_Quaternion(nn.Module):
    d_model: int; dtype: Any = DTYPE
    def setup(self):
        self.image_encoder=ImageEncoder(d_model=self.d_model,name="image_encoder",dtype=self.dtype);self.text_encoder=TextEncoder(d_model=self.d_model,name="text_encoder",dtype=self.dtype)
        self.w_gen=QuaternionComponentGenerator(d_model=self.d_model,name="w_gen",dtype=self.dtype);self.x_gen=QuaternionComponentGenerator(d_model=self.d_model,name="x_gen",dtype=self.dtype)
        self.y_gen=QuaternionComponentGenerator(d_model=self.d_model,name="y_gen",dtype=self.dtype);self.z_gen=QuaternionComponentGenerator(d_model=self.d_model,name="z_gen",dtype=self.dtype)
        self.topological_decoder=TopologicalGenerativeDecoder(d_model=self.d_model,name="topological_decoder",dtype=self.dtype);self.image_decoder=ImagePatchDecoder(self.d_model,name="image_decoder",dtype=self.dtype)
    def apply_quaternion_rotation(self,q:jnp.ndarray,v:jnp.ndarray)->jnp.ndarray:
        v_3d=v[:,:3];v_quat_3d=jnp.concatenate([jnp.zeros((v_3d.shape[0],1)),v_3d],axis=-1);q_conj=q*jnp.array([1.,-1.,-1.,-1.])
        def quat_mul(q1,q2):
            w1,x1,y1,z1=q1[...,0],q1[...,1],q1[...,2],q1[...,3];w2,x2,y2,z2=q2[...,0],q2[...,1],q2[...,2],q2[...,3]
        ...
        rotated_v_quat=quat_mul(quat_mul(q,v_quat_3d),q_conj);rotated_v_3d=rotated_v_quat[...,1:];return v.at[:,:3].set(rotated_v_3d)
    def generate_quaternion(self,z_initial):
        qw=self.w_gen(z_initial);qx=self.x_gen(z_initial);qy=self.y_gen(z_initial);qz=self.z_gen(z_initial);q=jnp.stack([qw,qx,qy,qz],axis=-1)
        return q/(jnp.linalg.norm(q,axis=-1,keepdims=True)+1e-8)
    def decode_from_z(self,z):generated_patches=self.topological_decoder(z);return self.image_decoder(generated_patches)
    def __call__(self,batch):
        images,gt_text_embs=batch['images'],batch['clip_text_embeddings'];z_initial=self.image_encoder(images);z_text=self.text_encoder(gt_text_embs)
        q_normalized=self.generate_quaternion(z_initial);z_rotated=self.apply_quaternion_rotation(q_normalized,z_initial);recon_images=self.decode_from_z(z_rotated)
        loss_align=jnp.mean((z_rotated-z_text)**2);loss_pixel=jnp.mean(jnp.abs(images-recon_images));total_loss=loss_align+loss_pixel
        aux={'loss':total_loss,'pixel_loss':loss_pixel,'align_loss':loss_align};return total_loss,aux

# =================================================================================================
# 4. DATA HANDLING
# =================================================================================================
def prepare_data(image_dir: str):
    base_path=Path(image_dir);record_file=base_path/"data_64x64.tfrecord";text_emb_file=base_path/"clip_text_embeddings.pkl";info_file=base_path/"dataset_info.pkl"
    if record_file.exists() and text_emb_file.exists():print(f"✅ All necessary data files exist in {image_dir}. Skipping.");return
    print(f"--- Preparing data from {image_dir} ---");image_paths=sorted([p for p in base_path.rglob('*') if p.suffix.lower() in ('.png','.jpg','.jpeg','.webp')])
    ...
    with tf.io.TFRecordWriter(str(record_file)) as writer:
        for i in tqdm(range(0,len(image_paths),256),desc="Processing Batches"):
            # This logic is slightly inefficient, let's correct it to iterate once
            batch_pairs = list(zip(image_paths, text_paths))[i:i+256]
            if not batch_pairs: continue
            batch_img_paths, batch_text_paths = zip(*batch_pairs)
            
            for img_path in batch_img_paths:
                img=Image.open(img_path).convert("RGB").resize((64,64),Image.Resampling.BICUBIC);img_bytes=tf.io.encode_jpeg(np.array(img),quality=95).numpy()
                ex=tf.train.Example(features=tf.train.Features(feature={'image':tf.train.Feature(bytes_list=tf.train.BytesList(value=[img_bytes]))}));writer.write(ex.SerializeToString())
            
            captions=[p.read_text().strip() for p in batch_text_paths];text_tokens=clip.tokenize(captions,truncate=True).to(_clip_device)
    ...
    with open(info_file,'wb') as f:pickle.dump({'num_samples':len(image_paths)},f);print(f"✅ Data preparation complete.")
...
def create_dataset(image_dir:str,batch_size:int):
    ...
    if not all([f.exists() for f in [record_file,text_emb_file]]):raise FileNotFoundError("Dataset files not found. Run 'prepare-data' first.")
    ...
    text_emb_ds=tf.data.Dataset.from_tensor_slices(clip_text_embs);full_ds=tf.data.Dataset.zip((img_ds,text_emb_ds))
    return full_ds.shuffle(1024).repeat().batch(batch_size,drop_remainder=True).map(lambda i,c_t:{'images':i,'clip_text_embeddings':c_t}).prefetch(tf.data.AUTOTUNE)

# =================================================================================================
# 5. THE MARKETPLACE TRAINER - VRAM MAX-SAVING VERSION
# =================================================================================================
class MarketplaceTrainer_Quaternion:
    def __init__(self, args):
        self.args=args;self.num_devices=jax.local_device_count();self.should_shutdown=False;signal.signal(signal.SIGINT,lambda s,f:setattr(self,'should_shutdown',True))
        self.vendor_count=args.vendor_count;self.market_depth=4;self.model=ChimeraAE_Quaternion(d_model=args.d_model);self.spec_names=['w_gen','x_gen','y_gen','z_gen']
    ...
    def _get_sparkline(self, data: deque, w=50):
        s = " ▂▃▄▅▆▇█"
        hist = np.array([val for val in data if np.isfinite(val)])
    ...
    def train(self):
        ...
        if ckpt_path.exists():
            ...
            print("--- Initializing new Quaternion model for Marketplace training ---");key,init_key=jax.random.split(key)
        ...
        def train_step(state, key, batch):
            ...
            spec_keys={name:jax.random.split(jax.random.fold_in(key,i),self.vendor_count) for i,name in enumerate(self.spec_names)}
            path_indices=jnp.stack(jnp.meshgrid(*[jnp.arange(self.vendor_count)]*self.market_depth),-1).reshape(-1,self.market_depth)
            def get_delta_for_vendor(component_name,vendor_idx,base_param_tree):
                def generate_leaf(p):
                    positive_hash=hash(p.shape)%(2**32)
                    leaf_key=jax.random.fold_in(spec_keys[component_name][vendor_idx],positive_hash)
                    return jax.random.normal(leaf_key,p.shape,dtype=p.dtype)
                return jax.tree_util.tree_map(generate_leaf,base_param_tree)
            def loss_scan_body(carry,path_idx_vector):
                mutable_params=unfreeze(base_params)
                for i,name in enumerate(self.spec_names):
                    vendor_idx=path_idx_vector[i]
                    delta=get_delta_for_vendor(name,vendor_idx,base_params[name])
                    perturbed_params=jax.tree_util.tree_map(lambda b,d:b+d,base_params[name],delta)
                    mutable_params[name]=perturbed_params
                loss,_=self.model.apply({'params':mutable_params},batch);return carry,loss
            _,all_losses=jax.lax.scan(loss_scan_body,None,path_indices)
            mean_loss,std_loss=jnp.mean(all_losses),jnp.std(all_losses)+1e-8
            attributions=-(all_losses-mean_loss)/std_loss
            zero_grads={name:jax.tree_util.tree_map(jnp.zeros_like,base_params[name]) for name in self.spec_names}
            def update_scan_body(accumulated_grads,path_data):
                path_idx_vector,attribution=path_data
                for i,name in enumerate(self.spec_names):
                    vendor_idx=path_idx_vector[i]
                    delta=get_delta_for_vendor(name,vendor_idx,base_params[name])
                    update_contribution=jax.tree_util.tree_map(lambda d:d*attribution,delta)
                    accumulated_grads[name]=jax.tree_util.tree_map(lambda acc,con:acc+con,accumulated_grads[name],update_contribution)
                return accumulated_grads,None
            reconciled_deltas,_=jax.lax.scan(update_scan_body,zero_grads,(path_indices,attributions))
            updated_spec_params={}
            for name in self.spec_names:
                grad_approx=jax.tree_util.tree_map(lambda d:d/path_indices.shape[0],reconciled_deltas[name])
                updated_spec_params[name]=jax.tree_util.tree_map(lambda p,g:p+g*lr,base_params[name],grad_approx)
            new_base_params=unfreeze(base_params);new_base_params.update(updated_spec_params)
            metrics=jax.lax.pmean({'avg_loss':mean_loss,'min_loss':jnp.min(all_losses),'std_loss':std_loss},'devices')
            new_state={'base_params':freeze(new_base_params)};return new_state,metrics
        ...
        layout["main"].split_row(Layout(name="left"),Layout(name="right",ratio=2));progress=Progress(TextColumn("[bold]Epoch {task.fields[epoch]}/{task.fields[total_epochs]}"),BarColumn(),"[p.p.]{task.percentage:>3.1f}%","•",TimeRemainingColumn(),"•",TimeElapsedColumn())
        ...
        with Live(layout,screen=True,redirect_stderr=False,vertical_overflow="visible") as live:
            for epoch in range(start_epoch,self.args.epochs):
                ...
                for step in range(steps_per_epoch):
                    ...
                    key,*train_keys=jax.random.split(key,self.num_devices+1);sharded_keys=jax.device_put_sharded(train_keys,jax.local_devices())
                    ...
                    if step%5==0:
                        ...
                        layout["left"].update(Panel(stats_tbl,title="[bold]📊 Stats[/]",border_style="blue"));spark_w=max(1,(live.console.width*2//3)-25)
...
class Generator:
    ...
    def transform(self):
        ...
        img_np=(np.array(img,dtype=np.float32)/127.5)-1.0;current_z=self._get_z_from_image(jnp.expand_dims(img_np,axis=0))
        q_operator=self._get_quaternion(current_z);frames=[]
        for step in tqdm(range(self.args.steps),desc="Animating frames"):
            ...
            frames.append(Image.fromarray(img_out_np).resize((256,256),Image.Resampling.NEAREST));current_z=self._rotate_z(q_operator,current_z)
        base_slug=Path(self.args.base_image).stem;target_slug="".join(c if c.isalnum() else "_" for c in self.args.target_prompt).strip("_")[:25]
        save_path=f"transform_{base_slug}_to_{target_slug}_{self.args.seed}.gif"
        frames[0].save(save_path,save_all=True,append_images=frames[1:],optimize=False,duration=150,loop=0);print(f"✅ Animation saved to {save_path}")
...
def main():
    ...
    subparsers=parser.add_subparsers(dest="command",required=True);p_prep=subparsers.add_parser("prepare-data",help="Prepare images and CLIP text embeddings.")
    p_prep.add_argument('--image-dir',type=str,required=True);p_train=subparsers.add_parser("train",help="Train the Quaternion Dynamics model using Marketplace.")
    ...
    p_train.add_argument('--epochs',type=int,default=100);p_train.add_argument('--batch-size',type=int,default=1,help="Per device. Keep this low due to high computation per step.");p_train.add_argument('--lr',type=float,default=1e-3)
    p_train.add_argument('--vendor-count',type=int,default=8,help="Number of variations per component. 8^4=4096 paths.");p_train.add_argument('--seed',type=int,default=42)
    ...
    p_trans.add_argument('--steps',type=int,default=30,help="Number of frames in the animation.");p_trans.add_argument('--basename',type=str,required=True);p_trans.add_argument('--d-model',type=int,default=256)
