...
bool lfm2_forward(const lfm2_model_t *m, const float *emb, int B, int T, float *logits) {
    ...
    /* LFM2.5: hidden = embedding_norm(embed(tokens)) before the first layer.
     * Apply per-token RMSNorm so the residual stream starts at the model's
     * expected scale (matches the reference; raw embed is ~2x too large). */
    for (int t = 0; t < T; t++)
        lfm2_rmsnorm(h + (size_t)t * d, m->embed_norm, d, 1e-5f);
    ...
    /* final embedding norm + tied lm_head */
