...
bool lfm2_load(const char *model_dir, lfm2_model_t *m) {
    ...
    char *buf = (char *)xmalloc((size_t)sz + 1); fread(buf, 1, sz, f); buf[sz] = 0; fclose(f);
...
#undef GETI
    ...
    m->rope_theta = 10000.0f;
    {
        ...
        if (fp) {
            ...
            char *b = (char *)xmalloc((size_t)z + 1); fread(b, 1, z, fp); b[z] = 0; fclose(fp);
    ...
    /* layer_types from config.json */
    {
        ...
        char *cb = (char *)xmalloc((size_t)csz + 1); fread(cb, 1, csz, fc); cb[csz] = 0; fclose(fc);
