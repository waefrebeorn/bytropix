...
REM =====================================================================
REM NON-FUNCTIONAL COMMENTARY EXAMPLE SCRIPT for WuBuGAADHybridGen_v0.1.py - Phase 3: Enable Discriminator GAAD-FiLM UNVERIFIED
REM Author: WuBu
REM Date: 2025-05-14
REM WIP: Phase 2 focus rn, this file is for the nerds
REM =====================================================================

REM --- Determine Project Root and Python Executable ---
...
REM --- Core DDP/Path Settings ---
SET "NPROC_PER_NODE=1"
SET "MASTER_ADDR=localhost"
SET "MASTER_PORT=29516" REM Increment port for new phase
SET "DATA_DIR_BASE=%PROJECT_ROOT%\data"
SET "VIDEO_DATA_PATH=%DATA_DIR_BASE%\demo_video_data_dir"
SET "VALIDATION_VIDEO_PATH="

REM --- !!! KEY CHANGES FOR THIS PHASE (Phase 3) !!! ---
SET "CHECKPOINT_OUTPUT_DIR=%PROJECT_ROOT%\checkpoints\WuBuGAADHybridGen_v01_Phase3_DFiLM"
REM *** IMPORTANT: Update LOAD_CHECKPOINT to your best from Phase 2 (e.g., after lambda_kl=0.01) ***
SET "LOAD_CHECKPOINT=%PROJECT_ROOT%\checkpoints\WuBuGAADHybridGen_v01_Phase2b_KL01\wubugaad_hybridgen_ckpt_v01_best.pt" 
SET "LAMBDA_KL=0.01"                              REM << Assuming this was the stable value from end of Phase 2
SET "DISC_USE_GAAD_FILM_CONDITION=true"           REM << ENABLED Discriminator FiLM
SET "EPOCHS=15"                                   REM << Number of epochs for this phase
SET "WANDB_RUN_NAME=Phase3_DFiLM_on_KL0.01"       

REM --- Hyperparameters (Many will be loaded from checkpoint's args) ---
REM --- Start with LRs from the end of successful Phase 2 ---
SET "LEARNING_RATE_GEN=3e-4"
SET "LEARNING_RATE_DISC=1e-4" 
REM (Q-controller will adapt. If D struggles with FiLM, might consider slightly higher D_LR later)

REM --- Other settings generally kept from the previous successful configuration ---
SET "IMAGE_H=256"
SET "IMAGE_W=256"
SET "NUM_CHANNELS=3"
...
REM DISC_USE_GAAD_FILM_CONDITION is set above
...
REM WANDB_RUN_NAME is set above
SET "WANDB_LOG_TRAIN_RECON_INTERVAL=25"
SET "WANDB_LOG_FIXED_NOISE_INTERVAL=100"

REM --- Construct SCRIPT_ARGS (Each on a new line for clarity and correctness) ---
SET "SCRIPT_ARGS="
SET "SCRIPT_ARGS=%SCRIPT_ARGS% --video_data_path "%VIDEO_DATA_PATH%""
IF DEFINED VALIDATION_VIDEO_PATH (IF NOT "%VALIDATION_VIDEO_PATH%"=="" SET "SCRIPT_ARGS=%SCRIPT_ARGS% --validation_video_path "%VALIDATION_VIDEO_PATH%"")
IF DEFINED LOAD_CHECKPOINT (IF NOT "%LOAD_CHECKPOINT%"=="" SET "SCRIPT_ARGS=%SCRIPT_ARGS% --load_checkpoint "%LOAD_CHECKPOINT%"")
...
ECHO ======================================================
ECHO WuBuGAADHybridGen VAE-GAN - Phase 3: Enable D-FiLM (lambda_kl=%LAMBDA_KL%)
ECHO Python: %PYTHON_EXE%
ECHO Script Name: %SCRIPT_NAME%
ECHO Checkpoints Dir for this Phase: %CHECKPOINT_OUTPUT_DIR%
ECHO Loading Checkpoint: %LOAD_CHECKPOINT%
ECHO Motion Branch: %USE_WUBU_MOTION_BRANCH% (Optical Flow: %OPTICAL_FLOW_NET_TYPE%)
ECHO Discriminator GAAD FiLM: %DISC_USE_GAAD_FILM_CONDITION%
ECHO Learning Rate (Gen/Disc): %LEARNING_RATE_GEN% / %LEARNING_RATE_DISC%
ECHO Epochs for this phase: %EPOCHS%
ECHO WandB Run Name: %WANDB_RUN_NAME%
ECHO ======================================================
ECHO.

IF NOT EXIST "%PYTHON_EXE%" ( ECHO ERROR: Python executable not found at %PYTHON_EXE% & GOTO :End )
...
IF EXIST "%VENV_ACTIVATE_PATH%" (
    ...
    IF ERRORLEVEL 1 ( ECHO WARNING: Failed to activate venv, proceeding. )
) ELSE (
    ECHO INFO: No venv activate script found at %VENV_ACTIVATE_PATH%, assuming Python is in PATH.
)
ECHO.

IF NOT EXIST "%CHECKPOINT_OUTPUT_DIR%" ( ECHO Creating checkpoint directory for this phase: %CHECKPOINT_OUTPUT_DIR% & MKDIR "%CHECKPOINT_OUTPUT_DIR%" )
IF NOT EXIST "%DATA_DIR_BASE%" MKDIR "%DATA_DIR_BASE%"
IF NOT EXIST "%VIDEO_DATA_PATH%" MKDIR "%VIDEO_DATA_PATH%"
ECHO.

ECHO Starting training script: %SCRIPT_NAME% (Phase 3 - D-FiLM=%DISC_USE_GAAD_FILM_CONDITION%, lambda_kl=%LAMBDA_KL%)
ECHO Full command to be executed (for debugging):
...
ECHO.
IF %EXIT_CODE% NEQ 0 ( ECHO * SCRIPT FAILED with exit code %EXIT_CODE% * ) ELSE ( ECHO * SCRIPT FINISHED successfully * )
