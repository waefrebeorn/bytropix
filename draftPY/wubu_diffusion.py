...
def print_tensor_stats(tensor: Optional[torch.Tensor], name: str, micro_level_idx: Optional[int] = None, stage_idx: Optional[int] = None, enabled: bool = True, batch_idx_to_print: int = 0):
    ...
    item_tensor = tensor; prefix = f"L{micro_level_idx if micro_level_idx is not None else 'N'}/S{stage_idx if stage_idx is not None else 'N'}| "
...
# --- PoincareBall Class (Copied from WuBuCPUOnlyGen_v1.py) ---
class PoincareBall:
...
# --- Core WuBu Components (Copied from WuBuCPUOnlyGen_v1.py, with logger changes) ---
class SkewSymmetricMatrix(nn.Module):
    ...
    def forward(self) -> torch.Tensor:
        dev = self.params.device if self.params is not None else torch.device('cpu'); dtype = self.params.dtype if self.params is not None else torch.float32
        if self.n_dim <= 1 or self.params is None: return torch.eye(self.n_dim, device=dev, dtype=dtype)
        X = torch.zeros(self.n_dim, self.n_dim, device=dev, dtype=dtype); indices = torch.triu_indices(self.n_dim, self.n_dim, offset=1, device=dev)
        if X.numel() > 0 and indices.numel() > 0 and self.params is not None and self.params.numel() > 0:
             ...
             else: logger_wubu_diffusion.warning(f"SkewSymMtx: Mismatch triu_idx ({indices.shape[1]}) vs params ({self.params.shape[0]}) for dim {self.n_dim}."); num_to_fill = min(indices.shape[1], self.params.shape[0]);
        ...
        except Exception as e: logger_wubu_diffusion.error(f"matrix_exp error: {e}. Using identity. X norm: {X.norm().item() if X.numel()>0 else 'N/A'}", exc_info=True); R = torch.eye(self.n_dim, device=X.device, dtype=X.dtype)
        return R

class QuaternionSO4From6Params(nn.Module):
    def __init__(self, small_init_std: float = 0.001):
        super().__init__(); self.so3_params_for_p = nn.Parameter(torch.randn(3) * small_init_std); self.so3_params_for_q = nn.Parameter(torch.randn(3) * small_init_std)
    def _so3_params_to_unit_quaternion(self, so3_params: torch.Tensor) -> torch.Tensor:
        angle = torch.linalg.norm(so3_params, dim=-1); identity_q_val = torch.zeros(4, device=so3_params.device, dtype=so3_params.dtype)
        if identity_q_val.numel() > 0 : identity_q_val[0] = 1.0
        if angle < EPS : return identity_q_val
        axis = so3_params / (angle.unsqueeze(-1) + EPS); return quaternion_from_axis_angle(angle, axis)
    def forward(self) -> Tuple[torch.Tensor, torch.Tensor]: return self._so3_params_to_unit_quaternion(self.so3_params_for_p), self._so3_params_to_unit_quaternion(self.so3_params_for_q)
...
class FractalMicroTransformRungs(nn.Module):
    def __init__(self, dim: int = 4, transform_type: str = "mlp", hidden_dim_factor: float = 1.0, use_quaternion_so4: bool = True,
        ...
        if self.enable_hypernetwork_modulation and self.dim > 0:
            ...
            else: self.enable_hypernetwork_modulation = False; self.hyper_bias_generator = None # Ensure hyper_bias_generator is None if conditions not met
    ...
    def _apply_internal_quaternion_sub_processing(self, x_q: torch.Tensor, micro_level_idx: Optional[int], stage_idx: Optional[int]) -> torch.Tensor:
        ...
        print_tensor_stats(x_q, "ISP_In_x_q", micro_level_idx, stage_idx); w, v = x_q[..., 0:1], x_q[..., 1:4]; w_modulation_factor = self.w_modulator(w)
        ...
        if DEBUG_MICRO_TRANSFORM_INTERNALS and self.debug_counter % 10 == 0: print_tensor_stats(w_modulation_factor, "ISP_w_mod", micro_level_idx, stage_idx); print_tensor_stats(x_q_norm, "ISP_Out_Norm", micro_level_idx, stage_idx)
        return x_q_norm
    ...
    def _apply_non_rotational_map(self, x_in: torch.Tensor, original_main_tan_for_hypernet: torch.Tensor, micro_level_idx: Optional[int], stage_idx: Optional[int]) -> torch.Tensor:
        ...
        if self.enable_hypernetwork_modulation and self.hyper_bias_generator is not None and isinstance(first_linear_layer, nn.Linear):
            ...
            if DEBUG_MICRO_TRANSFORM_INTERNALS and self.debug_counter % 10 == 0: print_tensor_stats(dynamic_bias_offset, "Hyper_BiasOffset", micro_level_idx, stage_idx)
            ...
            temp_x = x_processed # Initialize temp_x correctly
    ...
    def forward(self, main_tan: torch.Tensor, current_internal_state_in: Optional[torch.Tensor]=None, scaffold_modulation: Optional[torch.Tensor]=None, micro_level_idx: Optional[int]=None, stage_idx: Optional[int]=None) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
        ...
        print_tensor_stats(main_tan, "MF_In_main", micro_level_idx, stage_idx); x_for_hypernet_mod = main_tan
        ...
        print_tensor_stats(x_intermediate, "MF_PostISP", micro_level_idx, stage_idx); rotated_x, R_matrix = self.apply_rotation(x_intermediate)
        ...
        mapped_x = self._apply_non_rotational_map(rotated_x, x_for_hypernet_mod, micro_level_idx, stage_idx); print_tensor_stats(mapped_x, "MF_PostMap", micro_level_idx, stage_idx)
        ...
        if self.enable_stateful_micro_transform and current_internal_state_in is not None and self.state_influence_mlp and self.state_update_gate_mlp:
            print_tensor_stats(current_internal_state_in, "STF_In_State", micro_level_idx, stage_idx); influence = self.state_influence_mlp(current_internal_state_in)
            print_tensor_stats(influence, "STF_Influence", micro_level_idx, stage_idx); final_tan_before_scaffold = mapped_x + influence
            ...
            print_tensor_stats(delta, "STF_Delta", micro_level_idx, stage_idx); next_state = torch.tanh(current_internal_state_in + delta)
        ...
        print_tensor_stats(final_tan, "MF_PostScaff", micro_level_idx, stage_idx); clamped_tan = torch.clamp(final_tan, -TAN_VEC_CLAMP_VAL, TAN_VEC_CLAMP_VAL)
...
class FractalDepthQuaternionWuBuRungs(nn.Module): # From WuBu_Integrated_Full (v...08)
    def __init__(self, input_dim: int, output_dim: int, num_virtual_micro_levels: int = 10000, base_micro_level_dim: int = 4, num_physical_transform_stages: int = 10,
                 ...
                 rung_modulation_strength: float = 0.05, t_tilde_activation_scale: float = 1.0, micro_transform_type: str = "mlp",
                 micro_transform_hidden_factor: float = 1.0, use_quaternion_so4_micro: bool = True, scaffold_co_rotation_mode: str = "none",
                 enable_internal_sub_processing: bool = True, enable_stateful_micro_transform: bool = True, enable_hypernetwork_modulation: bool = True,
                 internal_state_dim_factor: float = 1.0, hyper_mod_strength: float = 0.01):
        super().__init__(); self.logger_fractal_rungs = logger_wubu_diffusion; self.num_virtual_micro_levels = num_virtual_micro_levels
        self.base_micro_level_dim = base_micro_level_dim; self.enable_stateful_micro_transform_globally = enable_stateful_micro_transform
        if self.base_micro_level_dim < 0: self.base_micro_level_dim = 0
        if input_dim < 0: input_dim = 0
        if output_dim < 0: output_dim = 0
        if self.base_micro_level_dim != 4 and use_quaternion_so4_micro: self.logger_fractal_rungs.warning("use_quaternion_so4_micro=True but base_dim!=4. Forced off."); use_quaternion_so4_micro = False
        if self.base_micro_level_dim != 4 and enable_internal_sub_processing: self.logger_fractal_rungs.warning("enable_internal_sub_processing=True but base_dim!=4. Forced off."); enable_internal_sub_processing = False
        self.num_physical_transform_stages = max(1, num_physical_transform_stages)
        if self.num_virtual_micro_levels > 0 and self.num_physical_transform_stages > 0: self.micro_levels_per_stage = max(1, self.num_virtual_micro_levels // self.num_physical_transform_stages)
        else: self.micro_levels_per_stage = 1
        self.initial_s, self.s_decay_factor = initial_s, s_decay_factor_per_micro_level; self.initial_c_base, self.c_phi_influence = initial_c_base, c_phi_influence
        self.min_curvature, self.t_tilde_activation_scale = EPS, t_tilde_activation_scale; self.scaffold_co_rotation_mode = scaffold_co_rotation_mode
        if use_quaternion_so4_micro and scaffold_co_rotation_mode == "matrix_only": self.logger_fractal_rungs.warning("Scaffold co-rotation 'matrix_only' may not work with quat micro-transforms if they don't output R_matrix.")
        if input_dim > 0 and self.base_micro_level_dim > 0: self.input_projection = nn.Linear(input_dim, self.base_micro_level_dim) if input_dim != self.base_micro_level_dim else nn.Identity(); self.input_layernorm = nn.LayerNorm(self.base_micro_level_dim)
        elif input_dim == 0 and self.base_micro_level_dim > 0: self.input_projection = nn.Linear(1,self.base_micro_level_dim); self.input_layernorm = nn.LayerNorm(self.base_micro_level_dim)
        else: self.input_projection = nn.Identity(); self.input_layernorm = nn.Identity()
        if self.num_physical_transform_stages > 0 and self.base_micro_level_dim > 0:
            self.physical_micro_transforms = nn.ModuleList([FractalMicroTransformRungs(dim=self.base_micro_level_dim, transform_type=micro_transform_type, hidden_dim_factor=micro_transform_hidden_factor, use_quaternion_so4=use_quaternion_so4_micro, enable_internal_sub_processing=enable_internal_sub_processing, enable_stateful_micro_transform=enable_stateful_micro_transform, enable_hypernetwork_modulation=enable_hypernetwork_modulation, internal_state_dim_factor=internal_state_dim_factor, hyper_mod_strength=hyper_mod_strength) for _ in range(self.num_physical_transform_stages)])
        ...
        else: self.physical_micro_transforms = nn.ModuleList(); self.log_stage_curvatures_unconstrained = nn.ParameterList()
        self.num_phi_scaffold_points_per_stage = num_phi_scaffold_points_per_stage
        if self.num_phi_scaffold_points_per_stage > 0 and self.base_micro_level_dim > 0 and self.num_physical_transform_stages > 0: self.phi_scaffold_base_tangent_vectors = nn.ParameterList([nn.Parameter(torch.randn(num_phi_scaffold_points_per_stage, self.base_micro_level_dim) * phi_scaffold_init_scale_factor) for _ in range(self.num_physical_transform_stages)])
        else: self.phi_scaffold_base_tangent_vectors = None
        self.use_gaussian_rungs = use_gaussian_rungs; self.base_gaussian_std_dev_factor_rung, self.gaussian_std_dev_decay_factor_rung = base_gaussian_std_dev_factor_rung, gaussian_std_dev_decay_factor_rung
        ...
        in_feat_proj = self.input_projection.in_features if isinstance(self.input_projection, nn.Linear) else input_dim; out_feat_proj = self.output_projection.out_features if isinstance(self.output_projection, nn.Linear) else output_dim
        self.logger_fractal_rungs.info(f"FDQRungs Init: In {in_feat_proj}D -> {self.num_virtual_micro_levels} virtLvl ({self.base_micro_level_dim}D) over {self.num_physical_transform_stages} physStgs -> Out {out_feat_proj}D. Params: {param_count:,}. AdvMicro: SubP={enable_internal_sub_processing}, StateF={self.enable_stateful_micro_transform_globally}, HyperM={enable_hypernetwork_modulation}")

    def get_s_c_gsigma_at_micro_level(self, micro_level_idx: int, stage_idx: int) -> Tuple[float, float, float]:
        s_i = self.initial_s * (self.s_decay_factor ** micro_level_idx); s_i = max(EPS, s_i); c_i = self.initial_c_base
        if self.log_stage_curvatures_unconstrained and stage_idx < len(self.log_stage_curvatures_unconstrained):
            stage_c_unconstrained = self.log_stage_curvatures_unconstrained[stage_idx]; c_i = get_constrained_param_val(stage_c_unconstrained, self.min_curvature).item()
            if self.c_phi_influence and self.micro_levels_per_stage > 0 : micro_idx_in_stage = micro_level_idx % self.micro_levels_per_stage; phi_exp = (micro_idx_in_stage % 4) - 1.5; c_i *= (PHI ** phi_exp)
        c_i = max(self.min_curvature, c_i); sigma_gauss_i_skin = (self.base_gaussian_std_dev_factor_rung / max(s_i, EPS)) * (self.gaussian_std_dev_decay_factor_rung ** micro_level_idx)
        sigma_gauss_i_skin = max(EPS*100, sigma_gauss_i_skin); return s_i, c_i, sigma_gauss_i_skin
    ...
    def forward(self, x_input: torch.Tensor, show_progress: bool = False, progress_desc: Optional[str] = None) -> torch.Tensor:
        ...
        current_v_tan_fwd = self.input_projection(current_v_tan_fwd); current_v_tan_fwd = self.input_layernorm(current_v_tan_fwd)
        if self.num_virtual_micro_levels == 0 or not self.physical_micro_transforms: final_v_tan_fwd = current_v_tan_fwd
            ...
            s_at_stage_start_val: float = self.initial_s; batched_internal_micro_state: Optional[torch.Tensor] = None
            if self.enable_stateful_micro_transform_globally and self.base_micro_level_dim > 0 and len(self.physical_micro_transforms) > 0:
                ...
                if hasattr(first_micro_transform, 'enable_stateful_micro_transform') and first_micro_transform.enable_stateful_micro_transform and first_micro_transform.internal_state_dim > 0:
                    batched_internal_micro_state = torch.zeros(current_batch_size, first_micro_transform.internal_state_dim, device=current_v_tan_fwd.device, dtype=current_v_tan_fwd.dtype)
            ...
            for micro_i_fwd in micro_level_iterator:
                current_stage_idx_fwd = micro_i_fwd // self.micro_levels_per_stage; current_stage_idx_fwd = min(current_stage_idx_fwd, len(self.physical_micro_transforms)-1)
                ...
                if micro_idx_in_stage_fwd == 0:
                    ...
                    if self.enable_stateful_micro_transform_globally and self.base_micro_level_dim > 0 and hasattr(physical_transform_module_fwd, 'enable_stateful_micro_transform') and physical_transform_module_fwd.enable_stateful_micro_transform and physical_transform_module_fwd.internal_state_dim > 0:
                ...
                s_i_fwd, _c_i_fwd, sigma_gauss_skin_i_fwd = self.get_s_c_gsigma_at_micro_level(micro_i_fwd, current_stage_idx_fwd); scaffold_modulation_input: Optional[torch.Tensor] = None
                if self.use_gaussian_rungs and self.phi_scaffold_base_tangent_vectors is not None and self.num_phi_scaffold_points_per_stage > 0 and self.base_micro_level_dim > 0 and current_stage_idx_fwd < len(self.phi_scaffold_base_tangent_vectors):
                    ...
                    R_prop = accumulated_R_for_stage if self.scaffold_co_rotation_mode == "matrix_only" else None; propagated_scaffolds = self._propagate_scaffold_points(base_scaffolds, R_prop, s_i_fwd, s_at_stage_start_val)
                    diffs = current_v_tan_fwd.unsqueeze(1) - propagated_scaffolds.unsqueeze(0); sq_dists = torch.sum(diffs**2, dim=-1)
                    affinity_scores = torch.exp(-sq_dists / (2 * (sigma_gauss_skin_i_fwd**2) + EPS*100)); affinity_weights = F.softmax(affinity_scores / self.rung_affinity_temperature, dim=-1)
                    scaffold_modulation_input = self.rung_modulation_strength * torch.einsum('bn,nd->bd', affinity_weights, propagated_scaffolds)
                transformed_v_tan_micro, R_step, next_state = physical_transform_module_fwd(current_v_tan_fwd, batched_internal_micro_state, scaffold_modulation_input, micro_i_fwd, current_stage_idx_fwd)
        ...
        elif input_original_dim_fwd == 1: return output_features_fwd.squeeze(0) if output_features_fwd.shape[0] == 1 else output_features_fwd
...
def golden_subdivide_rect_fixed_n_cpu(frame_dims:Tuple[int,int], num_regions_target:int, dtype=torch.float, min_size_px=5) -> torch.Tensor:
    ...
    while rect_queue and len(all_rects) < num_regions_target * 3 :
        ...
        elif is_square_like and w_curr > min_size_px * PHI :
            cut_w = w_curr / PHI
            r1_w, r2_w = cut_w, w_curr - cut_w
            if r1_w >= min_size_px: children_coords.append({'x':x_off, 'y':y_off, 'w':r1_w, 'h':h_curr})
            if r2_w >= min_size_px: children_coords.append({'x':x_off + r1_w, 'y':y_off, 'w':r2_w, 'h':h_curr})
    ...
    for r_coords in all_rects:
        ...
        r_tensor = torch.tensor(r_coords, dtype=dtype, device=device); r_hashable = tuple(round(c, 2) for c in r_coords)
...
class RegionalPatchExtractorCPU(nn.Module):
    def __init__(self, patch_output_size: Tuple[int, int]):
        ...
        if patch_output_size[0] > 0 and patch_output_size[1] > 0: self.resize_transform = T.Resize(self.patch_output_size, interpolation=T.InterpolationMode.BILINEAR, antialias=True)
        else: self.resize_transform = None
    def forward(self, images: torch.Tensor, bboxes_batch: torch.Tensor) -> torch.Tensor: # images: B*N_frames, C, H, W; bboxes_batch: B*N_frames, N_regions, 4
        B_img, NumCh, H_img, W_img = images.shape; device, dtype = images.device, images.dtype; all_patches = []
        patch_h_out, patch_w_out = self.patch_output_size
        for i in range(B_img): # Iterate over each image in the effective batch (B*N_frames)
            img, bboxes = images[i], bboxes_batch[i]; current_img_patches = []
            for r in range(bboxes.shape[0]): # Iterate over regions for that image
                x1,y1,x2,y2 = bboxes[r].round().int().tolist(); x1c,y1c,x2c,y2c = max(0,x1),max(0,y1),min(W_img,x2),min(H_img,y2)
            ...
            all_patches.append(torch.stack(current_img_patches)) # Shape: [N_regions, C, patch_h, patch_w]
        return torch.stack(all_patches) # Shape: [B*N_frames, N_regions, C, patch_h, patch_w]
    ...
    def assemble_frames_from_patches(patches_batch:torch.Tensor, bboxes_batch:torch.Tensor, target_img_size:Tuple[int,int], out_range:Tuple[float,float]=(-1.0,1.0)) -> torch.Tensor:
        ...
        for b in range(B):
            for f in range(N_f):
                ...
                for r in range(N_r):
                    patch = patches_batch[b,f,r]; x1,y1,x2,y2 = bboxes_batch[b,f,r].float().round().int().tolist(); x1c,y1c,x2c,y2c = max(0,x1),max(0,y1),min(W_img,x2),min(H_img,y2)
...
class DiffusionProcess:
    def __init__(self, timesteps: int, beta_schedule: str = 'linear', beta_start: float = 0.0001, beta_end: float = 0.02, device: torch.device = torch.device("cpu")):
        ...
        self.alphas_cumprod_prev = F.pad(self.alphas_cumprod[:-1], (1, 0), value=1.0) # p_alphas_cumprod[0] = 1.0
        ...
        # clip  variance to prevent NaN issues (especially at t=0 where variance is undefined)
    ...
    def q_sample(self, x_start: torch.Tensor, t: torch.Tensor, noise: Optional[torch.Tensor] = None) -> torch.Tensor:
        """Diffuse the data (x_start) for t steps."""
    ...
    def p_mean_variance(self, model_output_eps: torch.Tensor, x_t: torch.Tensor, t: torch.Tensor, clip_denoised: bool = True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Get the posterior mean and variance for q(x_{t-1} | x_t, x_0) using model's noise prediction."""
    ...
    def p_sample(self, model_output_eps: torch.Tensor, x_t: torch.Tensor, t: torch.Tensor, clip_denoised: bool = True) -> torch.Tensor:
        """Sample x_{t-1} from the model output (predicted noise)."""
    ...
    def sample(self, model_callable: callable, batch_size: int, image_channels: int, image_size: Tuple[int,int],
               ...
               num_sampling_steps: Optional[int] = None, # Added num_sampling_steps
        ...
        if num_sampling_steps is not None and num_sampling_steps > 0 and num_sampling_steps < self.timesteps:
            # Create a strided sequence of timesteps, ensuring 0 is included.
            # Using linspace and then reversing is a common way to get a good distribution of steps.
            time_seq = torch.linspace(self.timesteps - 1, 0, num_sampling_steps, device=self.device).long()
            # Ensure uniqueness and sort if linspace produces duplicates due to rounding with small num_sampling_steps
            time_seq = torch.unique(time_seq, sorted=True).flip(dims=[0]) # flip to make it reversed for iteration
        ...
        for i_val_t in iterable_progress: # i_val_t is the actual timestep value
        ...
        x_0_image = ImageAssemblyUtilsCPU.assemble_frames_from_patches(
            x_0_patches.unsqueeze(1),
            sampling_bboxes.unsqueeze(1),
            image_size
        )
        return x_0_image
...
class WuBuDiffusionModel(nn.Module):
    def __init__(self, args: argparse.Namespace, video_config: Dict, gaad_config: Dict):
        super().__init__()
        self.args = args # Store args for access within the model
        ...
        self.wubu_noise_predictor_stack = FractalDepthQuaternionWuBuRungs(
            ...
            use_gaussian_rungs=args.wubu_use_gaussian_rungs,
        ...
        if self.use_bsp_gate:
            ...
            self.bsp_gate_mlp = nn.Sequential(
                nn.Linear(bsp_gate_input_dim, bsp_gate_hidden_dim),
                nn.GELU(),
            ...
            if hasattr(args, 'bsp_intermediate_dim') and args.bsp_intermediate_dim != wubu_output_dim_from_stack:
                 logger_wubu_diffusion.warning(f"BSP not used, but bsp_intermediate_dim ({args.bsp_intermediate_dim}) differs from wubu_output_dim_from_stack ({wubu_output_dim_from_stack}). Ensure FDQWR output_dim is correctly {wubu_output_dim_from_stack}.")
    ...
    def forward(self, x_t_flat_patches: torch.Tensor, time_t: torch.Tensor, current_global_step: Optional[int] = None) -> torch.Tensor:
        ...
        if self.time_mlp:
            ...
            if x_t_flat_patches.shape[0] == 1 and time_emb.shape[0] > 1:
                # This condition might occur if batch_size for training is > 1, but sampling uses batch_size=1
                # We need to ensure time_emb matches the batch dim of x_t_flat_patches
                if time_emb.shape[0] != x_t_flat_patches.shape[0]:
                    logger_wubu_diffusion.debug(f"Adjusting time_emb batch for single sample: from {time_emb.shape[0]} to {x_t_flat_patches.shape[0]}")
                    time_emb = time_emb[0:1, :].expand(x_t_flat_patches.shape[0], -1) # Take first, expand
            elif time_emb.shape[0] != x_t_flat_patches.shape[0]:
        ...
        if self.use_bsp_gate:
            ...
            if gate_input_for_bsp.shape[1] == 0 and isinstance(self.bsp_gate_mlp[0], nn.Linear) and self.bsp_gate_mlp[0].in_features == 1: # Check if gate MLP expects 1D
            ...
            if self.args.bsp_num_paths == 1:
                path_weights = torch.ones_like(path_logits)
            elif self.args.bsp_num_paths == 2 and self.args.bsp_gate_activation == 'sigmoid':
                path_weights_p1 = torch.sigmoid(path_logits[:,0:1]) # Assumes logits for path1 is first
                path_weights_p0 = 1.0 - path_weights_p1
                path_weights = torch.cat([path_weights_p0, path_weights_p1], dim=-1)
            else: # Default to Softmax for N paths
                 path_weights = F.softmax(path_logits, dim=-1)

            outputs_from_heads = []
            for i, head in enumerate(self.bsp_output_heads):
                outputs_from_heads.append(head(intermediate_or_final_features))

            stacked_head_outputs = torch.stack(outputs_from_heads, dim=0)
            weights_reshaped = path_weights.permute(1,0).unsqueeze(-1)
            predicted_noise_flat_patches = torch.sum(weights_reshaped * stacked_head_outputs, dim=0)
            ...
            if self.args.wandb and WANDB_AVAILABLE and wandb.run and \
               ...
               current_global_step % (self.args.log_interval * getattr(self.args, 'bsp_log_gate_interval_mult', 5)) == 0:
                 for p_idx in range(self.args.bsp_num_paths):
...
class VideoFrameDatasetCPU(Dataset): # Largely unchanged
    def __init__(self, video_path: str, num_frames_total: int, image_size: Tuple[int, int],
        ...
        else: self.logger.warning(f"Not enough frames ({self.num_disk_frames}) in '{self.video_path}' for sequence (needs {required_span_on_disk}). Dataset for mode '{self.mode}' will be empty."); self.samples_start_indices = []
        if not all_samples_start_indices: self.logger.error(f"No valid samples from '{self.video_path}' for any split."); self.samples_start_indices = []
        else:
            ...
            if val_fraction > 0.0 and val_fraction < 1.0:
                ...
                if self.mode == 'train' and data_fraction < 1.0 and len(self.samples_start_indices) > 1: num_to_keep = max(1, int(len(self.samples_start_indices) * data_fraction)); self.samples_start_indices = self.samples_start_indices[:num_to_keep]; self.logger.info(f"Using {data_fraction*100:.2f}% of available samples (no val split): {len(self.samples_start_indices)} samples.")
                elif self.mode == 'val': self.samples_start_indices = [] if val_fraction > 0 else all_samples_start_indices
        ...
        elif not self.samples_start_indices and self.mode == 'val' and val_fraction > 0.0 and val_fraction < 1.0 : self.logger.warning(f"VideoFrameDataset (VAL): No valid samples from '{self.video_path}' for validation split. Validation will be skipped.")
    ...
    def __getitem__(self, idx: int) -> torch.Tensor: # Returns a sequence of frames
        ...
        if not self.samples_start_indices or idx >= len(self.samples_start_indices):
            ...
            t_end_getitem = time.time(); duration = t_end_getitem - t_start_getitem; self.logger.error(f"__getitem__ (idx {idx}, worker {os.getpid()}, ERROR_OOB) from '{Path(self.video_path).name}' took {duration:.4f}s.")
        ...
        for i in range(self.num_frames_total_sequence):
            ...
            if actual_frame_idx_in_ram < self.num_disk_frames:
                try:
                    ...
                    elif frame_tensor_chw_uint8.shape[0] != 3 : self.logger.warning(f"Frame {actual_frame_idx_in_ram} has {frame_tensor_chw_uint8.shape[0]} channels. Using first 3 or repeating."); frame_tensor_chw_uint8 = frame_tensor_chw_uint8[:3,...] if frame_tensor_chw_uint8.shape[0] > 3 else frame_tensor_chw_uint8[0:1,...].repeat(3,1,1)
                    transformed_frame = self.normalize_transform(self.resize_transform(frame_tensor_chw_uint8).float()/255.0); frames_for_sample.append(transformed_frame)
                except Exception as e: self.logger.error(f"Error transforming frame {actual_frame_idx_in_ram} (sample {idx}): {e}", exc_info=True); frames_for_sample.append(torch.zeros((3, self.image_size[0], self.image_size[1]), dtype=torch.float32))
            else: self.logger.error(f"Frame index {actual_frame_idx_in_ram} out of bounds for disk_frames {self.num_disk_frames} (sample {idx}). Padding with zeros."); frames_for_sample.append(torch.zeros((3, self.image_size[0], self.image_size[1]), dtype=torch.float32))
        result_frames = torch.stack(frames_for_sample)
        if len(frames_for_sample) != self.num_frames_total_sequence: self.logger.error(f"Loaded {len(frames_for_sample)} frames, expected {self.num_frames_total_sequence} for sample {idx}. Padded. Result shape: {result_frames.shape}");
        if result_frames.shape[0] != self.num_frames_total_sequence: result_frames = torch.zeros((self.num_frames_total_sequence, 3, self.image_size[0], self.image_size[1]), dtype=torch.float32)
...
class CPUDiffusionTrainer:
    def __init__(self, model: WuBuDiffusionModel, diffusion_process: DiffusionProcess, args: argparse.Namespace):
        ...
        if os.path.exists(args.video_data_path) and os.path.isdir(args.video_data_path):
             ...
             if not video_files:
                 ...
                 if IMAGEIO_AVAILABLE and imageio is not None:
                     ...
                     # Determine sequence length needed for dataset
        ...
        self.train_loader = DataLoader( self.train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, pin_memory=False, drop_last=True ) # Added drop_last=True
        ...
        if args.val_fraction > 0.0 and args.val_fraction < 1.0:
            ...
            if len(self.val_dataset) > 0:
                self.val_loader = DataLoader(self.val_dataset, batch_size=args.val_batch_size, shuffle=False, num_workers=args.num_workers, pin_memory=False, drop_last=True) # Added drop_last=True
    ...
    def _log_samples_to_wandb(self, tag_prefix: str, frames_to_log: Optional[torch.Tensor], num_frames_per_sequence_to_log: int = 1, num_sequences_to_log_max: int = 2):
        ...
        # Ensure frames_to_log is 5D: (Batch, NumFramesInSeq, Channels, Height, Width)
        if current_frames_dim == 4: # (Batch, C, H, W) -> (Batch, 1, C, H, W)
            frames_to_log = frames_to_log.unsqueeze(1)
        
        if frames_to_log.ndim != 5: 
            self.logger.warning(f"WandB log samples: unexpected shape {frames_to_log.shape} (original: {current_frames_dim}D). Expected 5D (B, N_seq, C, H, W). Skip."); return
        
        B_log, N_seq_log, C_log, _, _ = frames_to_log.shape
        num_to_log_seqs = min(B_log, num_sequences_to_log_max) # Number of items from batch
        num_frames_log_this = min(N_seq_log, num_frames_per_sequence_to_log) # Number of frames from the sequence dimension

        wandb_imgs = []
        for b in range(num_to_log_seqs):
            for f_idx in range(num_frames_log_this):
                frame = frames_to_log[b,f_idx,...].cpu().float() # Get specific frame
                if C_log == 1: frame = frame.repeat(3,1,1) # Convert grayscale to RGB for wandb
                img_0_1 = (frame.clamp(-1,1)+1)/2.0 # Normalize to [0,1]
    ...
    def _validate_epoch(self):
        ...
        for batch_idx in val_prog_bar:
            try:
                val_frames_seq_raw = next(val_loader_iter_for_epoch)
            except StopIteration:
                self.logger.error("Validation DataLoader exhausted prematurely. This should not happen.")
                break
            ...
            if val_frames_seq.shape[1] < start_idx_for_x0 + num_frames_to_process_diffusion:
                self.logger.warning(f"Val: Not enough frames in sequence ({val_frames_seq.shape[1]}) for diffusion processing. Skipping batch.")
                continue
            ...
            for frame_idx_in_seq in range(x0_target_frames_batch.shape[1]): 
                ...
                image_level_epsilon_for_xt = torch.randn_like(x0_single_frame) # N(0,1) image-space noise
                ...
                if frame_bboxes is None and self.args.gaad_num_regions > 0:
                    self.logger.error("Validation bboxes are None when expected for frame processing.")
                ...
                # --- MODIFIED TARGET NOISE GENERATION for Validation ---
                # Generate N(0,1) noise directly in the shape of the patches.
                target_noise_N01_in_patch_space = torch.randn_like(x_t_patches) # Shape: (B, Nr, C, Ph, Pw)
            ...
            if batch_idx == 0 and self.args.num_val_samples_to_log > 0 and hasattr(self.args, 'val_sampling_log_steps') and self.args.val_sampling_log_steps > 0:
                if x0_target_frames_batch.numel() > 0: 
                    ...
                    sampled_val_frames = self.diffusion_process.sample(
                        model_lambda_for_sampling,
                        batch_size=num_samples_to_gen,
                        image_channels=self.args.num_channels,
                        image_size=(self.args.image_h, self.args.image_w),
                        patch_h=self.args.patch_size_h,
                        patch_w=self.args.patch_size_w,
                        num_regions=self.args.gaad_num_regions,
        ...
        if num_val_batches > 0:
            ...
            self.logger.info(f"Validation Epoch {self.current_epoch+1}: No batches processed.")
        self.model.train()
    ...
    def train(self, start_epoch: int = 0, initial_global_step: int = 0):
        ...
        for epoch in range(start_epoch, self.args.epochs):
            ...
            for batch_idx in prog_bar:
                ...
                try:
                    real_frames_seq_raw = next(train_loader_iter_for_epoch)
                except StopIteration:
                     self.logger.error("Train DataLoader exhausted prematurely. This should not happen if drop_last=True or loop is correct.")
                     break 
                ...
                for frame_idx_in_selected_seq in range(x0_frames_for_diffusion_batch.shape[1]): 
                    ...
                    image_level_epsilon_for_xt = torch.randn_like(x0_single_frame_train) # N(0,1) image-space noise
                    ...
                    if frame_bboxes_train is None and self.args.gaad_num_regions > 0:
                        self.logger.error("Train bboxes are None when expected for frame processing.")
                    ...
                    # --- MODIFIED TARGET NOISE GENERATION for Training ---
                    # Generate N(0,1) noise directly in the shape of the patches.
                    target_noise_N01_in_patch_space = torch.randn_like(x_t_patches_train) # Shape: (B, Nr, C, Ph, Pw)
                ...
                if num_frames_processed_train_batch_item > 0:
                    ...
                    avg_loss_for_batch_item = torch.tensor(0.0) 
                ...
                if self.global_step % self.args.log_interval == 0:
                    log_items = {"Loss": avg_loss_for_batch_item.item(), "T_DataLoad": time_dataload, "T_ToDevice": time_todevice,
                                 "T_ModelFwd": model_timings.get("time_model_fwd",0), 
                                 "T_TrainCycle": time_train_cycle } 
                    ...
                    if self.args.wandb and WANDB_AVAILABLE and wandb.run:
                        wandb_log_data = {"train/loss_mse_avg_batch_item": avg_loss_for_batch_item.item(), 
                                          "time/data_load_batch_item": time_dataload, 
                                          "time/to_device_batch_item": time_todevice,
                ...
                if self.args.wandb_log_train_samples_interval > 0 and self.global_step % self.args.wandb_log_train_samples_interval == 0:
                    if x0_frames_for_diffusion_batch.numel() > 0 :
                        ...
                        sampled_train_img = self.diffusion_process.sample(
                            model_lambda_for_sampling_train,
                            batch_size=num_samples_to_gen_train,
                            ...
                            num_sampling_steps=getattr(self.args, 'train_sampling_log_steps', self.args.val_sampling_log_steps), 
            ...
            if self.args.wandb_log_fixed_noise_samples_interval > 0 and (epoch+1)%self.args.wandb_log_fixed_noise_samples_interval==0:
                ...
                fixed_noise_pixels = self.diffusion_process.sample(
                    model_lambda_for_fixed_noise,
                    batch_size=num_fixed_samples,
                    image_channels=self.args.num_channels, image_size=(self.args.image_h, self.args.image_w),
                    patch_h=self.args.patch_size_h, patch_w=self.args.patch_size_w, num_regions=self.args.gaad_num_regions, 
                    num_sampling_steps=getattr(self.args, 'fixed_noise_sampling_log_steps', self.args.val_sampling_log_steps), 
    ...
    def _save_checkpoint(self, epoch: int, is_final: bool = False):
        # Ensure args passed to save_checkpoint are serializable (convert Path objects to str)
        ...
        data = {
            ...
            'args': serializable_args # Save the modified or original args
        }
        fn = f"wubudiffusion_v1_{'final' if is_final else f'ep{epoch+1}_step{self.global_step}'}.pt"
        try:
            torch.save(data, Path(self.args.checkpoint_dir) / fn)
            self.logger.info(f"Checkpoint saved: {fn}")
        except Exception as e:
            self.logger.error(f"Error saving ckpt {fn}: {e}", exc_info=True)
    ...
    def load_checkpoint(self, ckpt_path_str: Optional[str]) -> Tuple[int,int]:
        if not ckpt_path_str or not os.path.exists(ckpt_path_str):
            self.logger.warning(f"Checkpoint '{ckpt_path_str}' not found. Starting fresh.")
            return 0,0
        try:
            ckpt = torch.load(ckpt_path_str, map_location=self.device)
            self.logger.info(f"Loaded checkpoint: {ckpt_path_str}")
        except Exception as e:
            self.logger.error(f"Failed to load ckpt {ckpt_path_str}: {e}. Fresh start.", exc_info=True)
            return 0,0

        # Load args from checkpoint, but allow current CLI args to override non-model-structure args
        if 'args' in ckpt and ckpt['args'] is not None:
            self.logger.info("Attempting to load args from checkpoint.")
            loaded_ckpt_args_dict = ckpt['args']
            current_args_dict = vars(self.args)
            
            # Critical model structure args should ideally match or warn heavily if different
            critical_structure_args = [
                'image_h', 'image_w', 'num_channels', 'gaad_num_regions', 
                'patch_size_h', 'patch_size_w', 'diffusion_time_embed_dim',
                'model_wubu_num_virtual_levels', 'model_wubu_base_micro_dim', 'model_wubu_num_physical_stages',
                'use_bsp_gate', 'bsp_num_paths', 'bsp_intermediate_dim' 
                # Add other FDQWR structural params if they affect layer dimensions/counts
            ]
            
            for k_ckpt, v_ckpt_loaded in loaded_ckpt_args_dict.items():
                # Convert Path objects back if they were saved as strings (if applicable to your args)
                # This example assumes args are simple types or already handled
                v_ckpt = v_ckpt_loaded

                if k_ckpt in critical_structure_args and k_ckpt in current_args_dict:
                    if str(current_args_dict[k_ckpt]) != str(v_ckpt): # Compare as strings to handle type variations
                        self.logger.warning(f"CRITICAL ARG MISMATCH for '{k_ckpt}': Ckpt='{v_ckpt}', Current CLI='{current_args_dict[k_ckpt]}'. This may cause issues loading model state_dict. Model might not load correctly or at all.")
                        # Optionally, you might decide to force using checkpoint's critical args or exit
                        # For now, we'll proceed but the user is warned.
                
                if k_ckpt not in current_args_dict: # Arg present in ckpt but not in current parser (new arg in code?)
                    setattr(self.args, k_ckpt, v_ckpt)
                    self.logger.info(f"  Arg '{k_ckpt}' loaded from ckpt: '{v_ckpt}' (added to current args)")
                # Allow CLI to override non-critical args. If it's a critical arg, warning already issued.
                # For non-critical, if CLI differs, it's an intentional override.
                ...
                # If values match or it's a critical arg where we are just warning, no specific log here unless verbose.

        model_state_dict = ckpt.get('model_state_dict')
        if model_state_dict:
            try:
                self.model.load_state_dict(model_state_dict, strict=True) # Try strict first
                self.logger.info("Successfully loaded model_state_dict (strict=True).")
                ...
                try:
                    ...
                    self.logger.info("Successfully loaded model_state_dict with strict=False. Some weights may have been uninitialized if model structure changed.")
                except Exception as e_non_strict:
                    self.logger.error(f"Failed to load model_state_dict even with strict=False: {e_non_strict}. Model weights will be random/default initialized.")
        
        optimizer_state_dict = ckpt.get('optimizer_state_dict')
        if optimizer_state_dict:
            try:
                self.optimizer.load_state_dict(optimizer_state_dict)
                self.logger.info("Successfully loaded optimizer_state_dict.")
            except ValueError as e_val: # Often due to parameter group size mismatch
                self.logger.error(f"Error loading optimizer_state_dict (likely parameter mismatch if model structure changed): {e_val}. Optimizer will start fresh.")
            except Exception as e_opt: # Other errors
                self.logger.error(f"Other error loading optimizer_state_dict: {e_opt}. Optimizer will start fresh.")
        ...
        # If resetting epoch, also reset global_step unless specifically told not to (though usually they go together)
...
def parse_diffusion_arguments():
    ...
    parser.add_argument('--num_input_frames', type=int, default=1, help="Frames from dataset sequence to use as x0 inputs (can be >1 for video diffusion).")
    parser.add_argument('--num_predict_frames', type=int, default=0, help="If >0, these frames (after num_input_frames) are used as x0 targets. If 0, num_input_frames are used.")
    ...
    parser.add_argument('--diffusion_time_embed_dim', type=int, default=128, help="Dimension for timestep embedding. 0 to disable.")


    # WuBu Core Common Params (for the noise predictor model)
    ...
    parser.add_argument('--wubu_enable_isp', type=bool_type, default=True); parser.add_argument('--wubu_enable_stateful', type=bool_type, default=True)
    parser.add_argument('--wubu_enable_hypernet', type=bool_type, default=True); parser.add_argument('--wubu_internal_state_factor', type=float, default=0.5)
    ...
    parser.add_argument('--val_sampling_log_steps', type=int, default=64, 
                        help="Number of steps for faster qualitative validation sampling log. Set to 0 or <=0 to disable sample logging during validation, or to self.args.diffusion_timesteps for full sampling.")
    parser.add_argument('--show_val_sample_progress', type=bool_type, default=True, 
                        help="Show tqdm progress bar for validation sampling generation.")
    ...
    if parsed_args.use_bsp_gate and parsed_args.bsp_intermediate_dim <=0:
        ...
        parsed_args.bsp_intermediate_dim = 256 # Or raise error
...
def main_cpu_diffusion():
    ...
    if not args.show_train_progress_bar:
        ...
        class _TqdmDummyArgDisabled:
            ...
            def __init__(self, iterable=None, *args_dummy, **kwargs_dummy):
                ...
                if desc and desc != _TqdmDummyArgDisabled._last_desc_printed:
                    print(f"{desc} (Progress bars disabled by args)")
        ...
        logger_wubu_diffusion.info("TQDM progress bars are ENABLED.")
    else:
    ...
    logger_wubu_diffusion.setLevel(logging.INFO)
    if not logging.getLogger().hasHandlers() or True:
        logging.basicConfig(level=logger_wubu_diffusion.level, format='%(asctime)s %(name)s:%(lineno)d %(levelname)s %(message)s', force=True)

    logger_wubu_diffusion.info(f"--- WuBuDiffusionV1 (Advanced FDQWR with optional BSP) ---"); logger_wubu_diffusion.info(f"Effective Args: {vars(args)}")
    random.seed(args.seed); os.environ['PYTHONHASHSEED'] = str(args.seed); np.random.seed(args.seed); torch.manual_seed(args.seed)

    if args.wandb and WANDB_AVAILABLE:
        run_name = args.wandb_run_name if args.wandb_run_name else f"wubudiff_adv_bsp_{datetime.now().strftime('%y%m%d_%H%M')}"
    ...
    try: trainer.train(start_epoch=start_epoch, initial_global_step=start_global_step)
        ...
        if hasattr(trainer, 'current_epoch'): trainer._save_checkpoint(epoch=trainer.current_epoch, is_final=True)
        if args.wandb and WANDB_AVAILABLE and wandb.run: wandb.finish()
        logger_wubu_diffusion.info("WuBuDiffusionV1 (Advanced FDQWR with optional BSP) script finished.")
