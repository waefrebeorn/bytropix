...
class HyperbolicUtils:
    ...
    def lorentz_tangent_to_poincare_tangent(tangent_L: torch.Tensor, point_L: torch.Tensor, c: Union[float, torch.Tensor]) -> torch.Tensor:
        ...
        x_L_0 = point_L[..., 0:1]
        x_L_spatial = point_L[..., 1:]
        u_L_0 = tangent_L[..., 0:1]
        u_L_spatial = tangent_L[..., 1:]
...
class HyperbolicInterLevelTransform(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, config: Dict[str, Any], level_idx: int):
        ...
        if self.transform_type == "mlp":
            layers = []
            current_d = mlp_input_dim
            for i in range(config.get("inter_level_mlp_layers", 2)):
                is_last = (i == config.get("inter_level_mlp_layers", 2) - 1)
                out_d = mlp_output_dim if is_last else max(mlp_output_dim // 2, mlp_input_dim // 2, mlp_input_dim, 1) # Ensure out_d is at least 1. Original: max(mlp_output_dim, mlp_input_dim // 2) might be too small
    ...
    def forward(self, parent_tangent_p: torch.Tensor, parent_origin_p: torch.Tensor, parent_curvature: torch.Tensor, child_curvature: torch.Tensor) -> torch.Tensor:
        if self.use_lorentz:
            ...
            # Calculate its square root (also a 0-dim scalar).
            sqrt_child_c = child_curvature.sqrt()
            
            # Calculate the value for the time component of the canonical origin in Lorentz model.
            # This value is also a 0-dim scalar.
            time_component_val = 1.0 / (sqrt_child_c + self.eps)
            
            # Expand the scalar time component value to shape (batch_size, 1).
            # This preserves the dtype and device of time_component_val (derived from child_curvature).
            ...
            projected_tangent_l = transformed_tangent_l # Assuming mlp_transform outputs correctly dimensioned Lorentz tangent vectors.
                                                       # No explicit projection onto tangent space of child_origin_l_canonical needed here,
                                                       # as we are defining child_tangent_p_at_origin using this.
                                                       # If transformed_tangent_l needed projection to be valid tangent at child_origin_l_canonical:
                                                       # projected_tangent_l = self.hyp_utils.project_lorentz_tangent(transformed_tangent_l, child_origin_l_canonical)
                                                       # (project_lorentz_tangent would need to be implemented if required)
            ...
            # If not using Lorentz, assume direct transformation of Poincare tangent vectors
...
class FullyHyperbolicWuBuNestingModel(nn.Module):
    def __init__(self, input_tangent_dim: int, config: Optional[Dict[str,Any]] = None):
        super().__init__()
        self.config = config.copy() if config is not None else DEFAULT_WUBU_TEXT_CONFIG.copy()
        ...
        self.inter_level_transforms = nn.ModuleList()
    ...
    def _determine_output_dim(self):
        ...
        if not dims_to_agg and self.num_levels > 0 :
            ...
            self.aggregated_output_dim = 1
            self.output_tangent_dim = 1
    ...
    def forward(self, initial_tangent_p: torch.Tensor, initial_origin_p: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if initial_origin_p is None:
            initial_origin_p = torch.zeros_like(initial_tangent_p)

        current_t_p = initial_tangent_p
        current_processing_origin_p = initial_origin_p

        level_tangent_outputs = []

        for i in range(self.num_levels):
            level_out_o_from_level, level_feat_t, level_curv = self.levels[i](current_t_p, current_processing_origin_p)
            level_tangent_outputs.append(level_feat_t)

            if i < self.num_levels - 1:
                origin_for_interlevel_transform = torch.zeros_like(level_feat_t)
                current_t_p = self.inter_level_transforms[i](
                    level_feat_t,
                    origin_for_interlevel_transform,
                    level_curv,
                    self.levels[i+1].curvature
                )
                current_processing_origin_p = torch.zeros_like(current_t_p)
                
        agg_levels_cfg = self.config.get("final_aggregation_levels", "all")
        agg_indices = list(range(self.num_levels)) if agg_levels_cfg=="all" else [idx for idx in agg_levels_cfg if 0<=idx<len(level_tangent_outputs)] # type: ignore
        if not agg_indices: agg_indices = list(range(len(level_tangent_outputs)))
        
        to_agg = [level_tangent_outputs[idx] for idx in agg_indices]
...
class DeepSeekR1TransfusionHead(AbstractETPTransfusionHead):
    def __init__(self, source_embedding_dim: int, wubu_tangent_dim: int, mlp_hidden_dim_ratio: float=2.0, num_mlp_layers: int=2, activation_fn: str="GELU", use_layer_norm: bool=True, dropout_rate: float=0.1):
        super().__init__(source_embedding_dim,wubu_tangent_dim)
        layers=[]
        curr_dim=source_embedding_dim
        hid_dim=int(source_embedding_dim*mlp_hidden_dim_ratio)
        for i in range(num_mlp_layers):
            is_last=(i==num_mlp_layers-1)
            out_d=wubu_tangent_dim if is_last else hid_dim
...
class WuBuToDeepSeekR1Decoder(AbstractETPDecoder):
    def __init__(self, wubu_tangent_dim: int, source_embedding_dim: int, mlp_hidden_dim_ratio: float=2.0, num_mlp_layers: int=2, activation_fn: str="GELU", use_layer_norm: bool=True, dropout_rate: float=0.1):
        ...
        layers=[]
        curr_dim=wubu_tangent_dim
        hid_dim=int(wubu_tangent_dim*mlp_hidden_dim_ratio)
        for i in range(num_mlp_layers):
            is_last=(i==num_mlp_layers-1)
            out_d=source_embedding_dim if is_last else hid_dim
            layers.append(nn.Linear(curr_dim,out_d))
            if not is_last:
                if use_layer_norm: layers.append(nn.LayerNorm(out_d))
                layers.append(getattr(nn,activation_fn.upper(),nn.GELU())());
...
class ETP_WuBuText_DS_R1_Sphere(nn.Module):
    def __init__(self, ds_r1_embedding_dim:int, wubu_initial_tangent_dim:int, wubu_core_config:Optional[Dict]=None, head_mlp_layers:int=2, head_mlp_ratio:float=2.0, decoder_mlp_layers:int=2, decoder_mlp_ratio:float=2.0, activation_fn:str="GELU", use_layer_norm:bool=True, dropout_rate:float=0.1):
        super().__init__()
        self.ds_r1_embedding_dim=ds_r1_embedding_dim
        ...
        if isinstance(actual_wubu_config.get("ball_class"),type):
            BallClass=actual_wubu_config["ball_class"]
            actual_wubu_config["ball"]=BallClass(eps=actual_wubu_config.get("eps",1e-7)) # type: ignore
...
def calculate_reconstruction_loss_p1(
    ...
    if loss_type == "mse": return F.mse_loss(reconstructed_embeddings, original_embeddings)
        ...
        sim_dim = 1 if reconstructed_embeddings.ndim > 1 else 0
...
def _pairwise_similarity_p1(batch: torch.Tensor, metric: str = "cosine") -> torch.Tensor:
...
def _normalize_matrix_p1(matrix: torch.Tensor) -> torch.Tensor:
...
def calculate_vector_space_preservation_loss_p1(
    ...
    if normalize_similarity_matrices:
        sim_source = _normalize_matrix_p1(sim_source)
        sim_wubu = _normalize_matrix_p1(sim_wubu)
...
def extract_ds_r1_sentence_embeddings_p1(
    ...
    for i in range(0, len(texts), batch_size):
        ...
        try:
            ...
            logger_embedding_extractor_p1.error(f"Error processing batch {i // batch_size + 1}: {e}")
            raise
...
def save_embeddings_p1(embeddings: List[np.ndarray], output_path: str, output_format: str = "numpy_list"):
    ...
    if output_format == "numpy_list":
        try:
            ...
            if output_dir and not os.path.exists(output_dir):
                ...
                logger_embedding_extractor_p1.info(f"Created directory: {output_dir}")
            ...
            logger_embedding_extractor_p1.error(f"Error saving with np.savez_compressed: {e}")
            raise
    elif output_format == "hdf5":
        try:
            if not H5PY_AVAILABLE:
                logger_embedding_extractor_p1.error("h5py not installed for HDF5 save.")
            ...
            logger_embedding_extractor_p1.error(f"Error saving HDF5: {e}")
            raise
    else:
        logger_embedding_extractor_p1.error(f"Unsupported output format: {output_format}")
...
def _safe_to_float32_numpy_array_local(
    ...
    """
    Safely tries to convert an item to a np.float32 numpy array.
    Logs a warning and returns None if conversion fails.
    """
    if item is None:
        return None
    try:
        # If item is already a numpy array, ensure it's numeric or can be cast to float32.
        if isinstance(item, np.ndarray) and np.issubdtype(item.dtype, np.number):
            return item.astype(np.float32) # Ensure it's specifically float32
        
        # For other types (lists of numbers, Python numbers, numeric strings, object arrays containing numbers/numeric strings)
        # np.asarray will attempt conversion.
        arr = np.asarray(item, dtype=np.float32)
        
        # If np.asarray succeeded, arr.dtype should be np.float32.
        # An additional check for safety, though np.asarray(..., dtype=float32) should fail for unconvertible objects.
        if arr.dtype == np.object_: # pragma: no cover
            logger_to_use.warning(
                f"Skipping item '{item_name_for_log}' from file {filepath_for_log} "
                f"as it resulted in an object array even after attempting conversion to float32. "
                f"Content preview: '{str(item)[:100]}...'"
            )
            return None
        return arr
    except ValueError:
        logger_to_use.warning(
            f"Skipping item '{item_name_for_log}' from file {filepath_for_log} "
            f"as it could not be converted to a float32 numpy array. "
            f"Content preview: '{str(item)[:100]}...'"
        )
        return None
    except TypeError: # Handles cases like np.asarray(None, dtype=np.float32) if None slips through
        logger_to_use.warning(
            f"Skipping item '{item_name_for_log}' from file {filepath_for_log} "
            f"due to TypeError during conversion (item might be None or an incompatible type). "
            f"Content preview: '{str(item)[:100]}...'"
        )
        return None

def load_embeddings_from_file_p1(filepath: str) -> List[np.ndarray]:
    if not os.path.exists(filepath) and not os.path.isdir(filepath):
        logger_datasets_p1.error(f"File or directory not found: {filepath}")
    ...
    if file_extension == ".npz":
        try:
            with np.load(filepath, allow_pickle=True) as data:
                if not data.files:
                    logger_datasets_p1.warning(f"NPZ file {filepath} is empty.")
                    return []

                keys_to_process = list(data.files)
                # Standard naming convention for np.savez_compressed(*args) is 'arr_0', 'arr_1', ...
                # If all keys follow this, sort numerically. Otherwise, sort alphabetically.
                ...
                for key in keys_to_process:
                    ...
                    # Check if item_from_npz is a container (e.g., an object array holding other arrays/items)
                    is_container = False
                    if isinstance(item_from_npz, np.ndarray) and item_from_npz.dtype == np.object_:
                        # An ndarray of objects. If it has dimensions, it's a container.
                        # If ndim is 0, it's a scalar object (e.g., np.array("string", dtype=object))
                        if item_from_npz.ndim > 0 and len(item_from_npz) > 0:
                            is_container = True
                    elif isinstance(item_from_npz, list): # A Python list saved directly
                        is_container = True
                    
                    if is_container:
                        logger_datasets_p1.debug(f"NPZ key '{key}' in {filepath} appears to be a container. Processing its elements.")
                        for i, sub_item in enumerate(item_from_npz):
                            processed_emb = _safe_to_float32_numpy_array_local(
                                sub_item, filepath, f"{key}[{i}]", logger_datasets_p1
                            )
                            if processed_emb is not None:
                                embeddings.append(processed_emb)
                    else:
                        # Treat item_from_npz as a single potential embedding (or metadata)
                        processed_emb = _safe_to_float32_numpy_array_local(
                            item_from_npz, filepath, key, logger_datasets_p1
                        )
                        if processed_emb is not None:
                            embeddings.append(processed_emb)
                
                if embeddings:
                    logger_datasets_p1.info(f"Loaded {len(embeddings)} valid embeddings from NPZ {filepath}.")
                else:
                    logger_datasets_p1.warning(
                        f"No valid embeddings found in NPZ file {filepath} after processing {len(data.files)} top-level item(s)."
                    )
        
        except Exception as e:
            logger_datasets_p1.error(f"Error loading NPZ {filepath}: {e}")
            raise ValueError(f"Could not load NPZ {filepath}: {e}") from e

    elif file_extension in [".hdf5", ".h5"]:
        if not H5PY_AVAILABLE:
            logger_datasets_p1.error("h5py required for HDF5.")
            raise ImportError("h5py required.")
        try:
            with h5py.File(filepath, 'r') as hf:
                ...
                for key in dataset_keys:
                    item = hf[key][:] # Load data from HDF5 dataset
                    processed_emb = _safe_to_float32_numpy_array_local(
                        item, filepath, key, logger_datasets_p1
                    )
                    if processed_emb is not None:
                        embeddings.append(processed_emb)
            if embeddings:
                logger_datasets_p1.info(f"Loaded {len(embeddings)} valid embeddings from HDF5 {filepath}.")
            else:
                logger_datasets_p1.warning(f"No valid embeddings found in HDF5 file {filepath} after processing.")
        except Exception as e:
            logger_datasets_p1.error(f"Error loading HDF5 {filepath}: {e}")
            raise ValueError(f"Could not load HDF5 {filepath}: {e}") from e
        ...
        try:
            ...
            if not npy_files:
                logger_datasets_p1.warning(f"No .npy files in {filepath}.")
                return []
            for f_name in npy_files:
                item = np.load(os.path.join(filepath, f_name), allow_pickle=True)
                processed_emb = _safe_to_float32_numpy_array_local(
                    item, os.path.join(filepath, f_name), f_name, logger_datasets_p1
                )
                if processed_emb is not None:
                    embeddings.append(processed_emb)
            if embeddings:
                logger_datasets_p1.info(f"Loaded {len(embeddings)} valid embeddings from .npy files in {filepath}.")
            else:
                logger_datasets_p1.warning(f"No valid embeddings found in .npy files in directory {filepath} after processing.")
        except Exception as e:
            logger_datasets_p1.error(f"Error loading .npy from {filepath}: {e}")
            raise ValueError(f"Could not load .npy from {filepath}: {e}") from e
            
    else:
        logger_datasets_p1.error(f"Unsupported file type: {file_extension} for {filepath}")
        raise ValueError(f"Unsupported file type: {file_extension}.")
    
    return embeddings

class DeepSeekR1EmbeddingDataset_P1(Dataset):
    def __init__(self, embeddings_file_A: str, embeddings_file_B: str):
        ...
        try:
            ...
            logger_datasets_p1.error(f"Failed loading embeddings: {e}")
            raise
        if not self.embeddings_A: logger_datasets_p1.warning(f"File A ({embeddings_file_A}) resulted in no valid embeddings.")
        if not self.embeddings_B: logger_datasets_p1.warning(f"File B ({embeddings_file_B}) resulted in no valid embeddings.")
        if not self.embeddings_A and not self.embeddings_B: logger_datasets_p1.error("Both embedding sources are empty or contained no valid data.")
        elif not self.embeddings_A or not self.embeddings_B: logger_datasets_p1.warning("One embedding source is empty or contained no valid data.")
    ...
    def __len__(self) -> int:
        ...
        return max(len_A, len_B) # Or min, depending on desired behavior with mismatched lengths
    ...
    def __getitem__(self, idx: int) -> Dict[str, Union[np.ndarray, None]]:
        ...
        if len_A == 0 and len_B == 0:
            # This case should ideally not be reached if __len__ is 0.
            # If len is >0 due to one list having items, but that one is exhausted by modulo,
            # then this path might be taken if idx is out of effective range for both.
            # However, DataLoader usually queries __len__ first.
            logger_datasets_p1.error("Attempting to get item from a dataset that effectively has zero length or both sources are exhausted.")
            # Return dummy data that matches expected structure but indicates error or emptiness.
            # Shape of None might be an issue. The loader expects tensors.
            # Consider raising IndexError if a valid item cannot be returned.
            raise IndexError(f"Index {idx} out of bounds for dataset with A_len={len_A}, B_len={len_B}")
        ...
        if len_A > 0:
            emb_A = self.embeddings_A[idx % len_A]
        if len_B > 0:
            emb_B = self.embeddings_B[idx % len_B]
        
        # Data is already np.float32 from load_embeddings_from_file_p1
        # No further np.asarray(..., dtype=np.float32) needed here.
        # The batch collate function will convert these to tensors.

        # If one source is empty, its corresponding emb will be None.
        # The trainer needs to handle cases where one of the embeddings might be None
        # if that's a valid scenario (e.g. reconstruction only needs source_A).
        # For Phase 1, only source_A is used.
        if emb_A is None and len_A > 0 : # Should not happen if len_A > 0
             logger_datasets_p1.error(f"emb_A is None for idx {idx} but len_A is {len_A}. This indicates an issue.")
             raise RuntimeError(f"emb_A is None for idx {idx} but len_A is {len_A}")
        
        # Phase 1 primarily uses 'source_A'. If it's None (because embeddings_A was empty),
        # this will likely cause issues downstream when converting to tensor.
        # The DataLoader and training loop expect valid data.
        if emb_A is None and (len_A == 0 and len_B > 0): # A is empty, B is not, using B as fallback for A? No, stick to A.
             logger_datasets_p1.warning(f"Source A is empty, returning None for source_A at index {idx}")
             # This will likely fail in the collate_fn or trainer.
             # It's better if __len__ reflects usable data pairs.
             # For Phase 1, if A is empty, len should be 0.
        
        # The problem was `np.asarray(str_metadata, dtype=np.float32)`.
        # Since `load_embeddings_from_file_p1` now ensures `self.embeddings_A/B`
        # only contain valid `np.ndarray` of `np.float32`, this part is fine.
        # The original `np.asarray(emb_A, dtype=np.float32)` can be removed.
...
DEFAULT_CONFIG_QLEARN_HYBRID_P1: Dict[str, Any] = {
...
class GradientStats_P1:
    ...
    def update(self, params_with_grad: List[torch.nn.Parameter], current_lr: Optional[float] = None) -> None:
        ...
        current_grad_mean=all_grads_flat.mean().item()
        current_grad_std=all_grads_flat.std().item()
        self.grad_means.append(current_grad_mean)
        self.grad_stds.append(current_grad_std)
    ...
    def load_state_dict(self, state_dict: Dict[str, Any]) -> None:
        ...
        self.ewma_norm=state_dict.get("ewma_norm",0.0)
        self.ewma_mean=state_dict.get("ewma_mean",0.0)
        self.ewma_std=state_dict.get("ewma_std",0.0)
        self.alpha=state_dict.get("alpha",self.alpha)

class HAKMEMQController_P1:
    def __init__(self, initial_lr: float, config: Optional[Dict[str, Any]] = None, logger_suffix: str = ""):
        ...
        self.q_table_size=int(self.config.get("q_table_size",10))
        self.q_table_size=max(1,self.q_table_size)
        ...
        self.learning_rate_q=float(self.config.get("learning_rate_q",0.1))
        self.discount_factor_q=float(self.config.get("discount_factor_q",0.9))
        self.exploration_rate_q=float(self.config.get("exploration_rate_q",0.1))
        self.lr_min=float(self.config.get("lr_min",1e-7))
        self.lr_max=float(self.config.get("lr_max",1e-1))
        self.loss_history:Deque[float]=deque(maxlen=int(self.config.get("metric_history_len",5)))
        self.loss_min=float(self.config.get("loss_min",0.0))
        self.loss_max=float(self.config.get("loss_max",10.0))
    ...
    def _get_current_state_idx(self,current_loss_val:Optional[float])->int:
        if current_loss_val is not None:
            return self._discretize_value(current_loss_val,self.loss_min,self.loss_max,self.q_table_size)
    ...
    def choose_action(self,current_loss_val:Optional[float]=None,params_with_grad:Optional[List[torch.nn.Parameter]]=None)->float:
        ...
        if random.random()<self.exploration_rate_q:
            ...
            with torch.no_grad():
                self.last_action_idx=torch.argmax(self.q_table[self.last_state_idx]).item()
    ...
    def load_state_dict(self,state_dict:Dict[str,Any])->None:
        ...
        if "q_table" in state_dict:
            ...
            if loaded_q.shape==self.q_table.shape:
                self.q_table=loaded_q
            else:
                logger_optimizer_utils_p1.warning(f"HAKMEM ({self.logger_suffix}): Q-table shape mismatch. Not loading.")
...
class RiemannianEnhancedSGD_P1(optim.Optimizer):
    ...
    def step(self, closure: Optional[Any] = None) -> Optional[torch.Tensor]: # type: ignore[override]
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('RiemannianSGD does not support sparse gradients for now.')
                ...
                if hasattr(p, 'manifold') and p.manifold is not None:
                    ...
                    if hasattr(manifold, 'egrad2rgrad') and callable(manifold.egrad2rgrad):
                        ...
                        logger_optimizer_utils_p1.warning(f"P:{p.shape} manifold no egrad2rgrad. Euclidean grad.")
                    ...
                    if hasattr(manifold, 'expmap') and callable(manifold.expmap):
                        ...
                        logger_optimizer_utils_p1.warning(f"P:{p.shape} manifold no expmap. Euclidean update.")
...
class ETPTrainerPhase1:
    def __init__(self,
        ...
        self.optimizer_kwargs_wubu_core = optimizer_kwargs_wubu_core or {}
        self.optimizer_kwargs_mlps = optimizer_kwargs_mlps or {}
        ...
        _default_q_config_local_p1 = DEFAULT_CONFIG_QLEARN_HYBRID_P1.copy()
        ...
        self.wandb_run = None
        if wandb_project and WANDB_AVAILABLE and wandb is not None:
            try:
                ...
                logger_trainer_phase1.error(f"WandB init failed P1: {e_wandb_ph1}. Disabling.")
                self.wandb_run = None
    ...
    def _setup_optimizers_and_q_controllers(self) -> None:
        ...
        self.q_controllers: Dict[str, Optional[HAKMEMQController_P1]] = {"sphere_wubu_core": None, "sphere_mlps": None}
        self.optimizer_sphere_wubu_core = RiemannianEnhancedSGD_P1(
            wubu_core_params_list if wubu_core_params_list else [nn.Parameter(torch.zeros(1, requires_grad=True))], lr=self.lr_sphere_wubu_core,
            q_learning_config=self.q_config_sphere_wubu_core if self.q_controller_enabled else None,
            q_logger_suffix="SphereWuBuCoreP1", **self.optimizer_kwargs_wubu_core) # type: ignore
        ...
        self.optimizer_sphere_mlps = optim.AdamW(mlp_params_list if mlp_params_list else [nn.Parameter(torch.zeros(1, requires_grad=True))], lr=self.lr_sphere_mlps, **self.optimizer_kwargs_mlps)
    ...
    def _train_step(self, batch: Dict[str, torch.Tensor]) -> Tuple[Dict[str, float], Optional[torch.Tensor]]:
        ...
        with torch.cuda.amp.autocast(enabled=self.use_amp):
            ...
            loss_rec_tensor = calculate_reconstruction_loss_p1(reconstructed_A, embeddings_A)
            ...
            if self.lambda_vsp > 0:
                ...
                loss_vsp_tensor = calculate_vector_space_preservation_loss_p1(embeddings_A, latents_A)
                raw_losses_dict['loss_vsp'] = loss_vsp_tensor.item()
                if loss_total_tensor is not None:
                    loss_total_tensor = loss_total_tensor + (self.lambda_vsp * loss_vsp_tensor)
                else:
                    loss_total_tensor = self.lambda_vsp * loss_vsp_tensor
    ...
    def train_epoch(self) -> Dict[str,float]:
        ...
        for batch_idx, batch_data in enumerate(self.train_loader): # type: ignore
            ...
            if loss_total_tensor is not None:
                (self.scaler_sphere.scale(loss_total_tensor / self.grad_accum_steps) # type: ignore
                 .backward())
            ...
            if (batch_idx + 1) % self.grad_accum_steps == 0:
                ...
                if self.log_interval > 0 and self.global_step % self.log_interval == 0:
                    avg_bt = sum(batch_times)/len(batch_times) if batch_times else 0
                    batch_times=[]
                    avg_losses = {f"train_p1/{k}": v_sum/num_batches_this_epoch for k,v_sum in epoch_losses_sum.items()}
                    log_metrics = {**avg_losses, "train_p1/lr_wubu_core": self.optimizer_sphere_wubu_core.param_groups[0]['lr'],
                                   "train_p1/lr_mlps": self.optimizer_sphere_mlps.param_groups[0]['lr'],
                                   "train_p1/avg_batch_time_ms": avg_bt*1000, "progress/global_step": self.global_step,
                                   "progress/epoch": self.current_epoch+1}
                    logger_trainer_phase1.info(f"Epoch {self.current_epoch+1} | Step {self.global_step} | " +
                                               " | ".join([f"{k.split('/')[-1]}:{v:.4f}" for k,v in avg_losses.items() if v is not None]))
    ...
    def validate_epoch(self) -> Dict[str, float]:
        ...
        val_losses_sum = defaultdict(float)
        num_val_batches = 0
        ...
        with torch.no_grad():
            for batch_data in self.val_loader: # type: ignore
                ...
                with torch.cuda.amp.autocast(enabled=self.use_amp):
                    reconstructed_A = self.etp_sphere_model(embeddings_A)
                    val_losses_sum['val_p1_loss_rec'] += calculate_reconstruction_loss_p1(reconstructed_A, embeddings_A).item()
                    if self.lambda_vsp > 0:
                        latents_A = self.etp_sphere_model.get_latent(embeddings_A)
                        val_losses_sum['val_p1_loss_vsp'] += calculate_vector_space_preservation_loss_p1(embeddings_A, latents_A).item()
                num_val_batches +=1
        ...
        if self.best_val_metric_name not in avg_val_losses:
            avg_val_losses[self.best_val_metric_name] = avg_val_losses.get('val_p1_loss_rec',
                                                                           float('inf') if not self.best_val_metric_higher_is_better else float('-inf'))
        logger_trainer_phase1.info(f"Validation P1 Epoch {self.current_epoch}: " +
                                   " | ".join([f"{k}:{v:.4f}" for k,v in avg_val_losses.items()]))
        if self.wandb_run:
    ...
    def _save_checkpoint(self, is_best: bool = False, reason: str = "") -> None:
        ...
        try:
            ...
            logger_trainer_phase1.info(f"Saving Phase 1 checkpoint to {filepath}")
        except Exception as e:
            logger_trainer_phase1.error(f"Failed to save checkpoint to {filepath}: {e}")


    def load_checkpoint(self, path: str, load_optimizers: bool = True, load_q_controllers: bool = True, load_scaler: bool = True) -> None:
        filepath = Path(path)
        if not filepath.exists():
            logger_trainer_phase1.error(f"Checkpoint file not found: {filepath}")
            return

        logger_trainer_phase1.info(f"Attempting to load Phase 1 checkpoint from {filepath}")
        try:
            ckpt: Dict[str, Any] = torch.load(filepath, map_location=self.device)
        except Exception as e:
            logger_trainer_phase1.error(f"Error loading checkpoint file {filepath}: {e}")
            return

        if 'etp_sphere_model_state_dict' in ckpt:
            self.etp_sphere_model.load_state_dict(ckpt['etp_sphere_model_state_dict'])
            logger_trainer_phase1.info("Loaded etp_sphere_model state.")
        else:
            logger_trainer_phase1.warning("etp_sphere_model_state_dict not found in checkpoint.")

        if load_optimizers:
            if 'optimizer_sphere_wubu_core_state_dict' in ckpt and hasattr(self.optimizer_sphere_wubu_core, 'load_state_dict'):
                self.optimizer_sphere_wubu_core.load_state_dict(ckpt['optimizer_sphere_wubu_core_state_dict'])
                logger_trainer_phase1.info("Loaded optimizer_sphere_wubu_core state.")
            else:
                logger_trainer_phase1.warning("optimizer_sphere_wubu_core_state_dict not found or optimizer cannot load state.")
            
            if 'optimizer_sphere_mlps_state_dict' in ckpt and hasattr(self.optimizer_sphere_mlps, 'load_state_dict'):
                self.optimizer_sphere_mlps.load_state_dict(ckpt['optimizer_sphere_mlps_state_dict'])
                logger_trainer_phase1.info("Loaded optimizer_sphere_mlps state.")
            else:
                logger_trainer_phase1.warning("optimizer_sphere_mlps_state_dict not found or optimizer cannot load state.")

        if load_scaler and 'scaler_sphere_state_dict' in ckpt and hasattr(self.scaler_sphere, 'load_state_dict'):
            self.scaler_sphere.load_state_dict(ckpt['scaler_sphere_state_dict'])
            logger_trainer_phase1.info("Loaded scaler_sphere state.")
        elif load_scaler:
            logger_trainer_phase1.warning("scaler_sphere_state_dict not found or scaler cannot load state.")

        if load_q_controllers:
            for qc_name, qc_instance in self.q_controllers.items():
                key = f'q_controller_{qc_name}_state_dict'
                if qc_instance and key in ckpt:
                    qc_instance.load_state_dict(ckpt[key])
                    logger_trainer_phase1.info(f"Loaded q_controller_{qc_name} state.")
                elif qc_instance:
                     logger_trainer_phase1.warning(f"{key} not found for {qc_name}.")
        ...
        logger_trainer_phase1.info(f"Phase 1 Checkpoint loaded. Resuming from epoch {self.current_epoch + 1}, global step {self.global_step}.")
    ...
    def train(self, resume_from_checkpoint: Optional[str] = None) -> None:
        ...
        for epoch_iter in range(initial_completed_epochs, self.epochs):
            ...
            if self.q_controller_enabled:
                for opt_name, _ in self.q_controllers.items():
            ...
            if self.val_loader and (self.current_epoch % self.val_interval_epochs == 0 or self.current_epoch == self.epochs):
                ...
                current_val_metric = val_metrics.get(self.best_val_metric_name,
                                                    float('-inf') if self.best_val_metric_higher_is_better else float('inf'))
                is_better = ((current_val_metric > self.best_val_metric) if self.best_val_metric_higher_is_better
                             else (current_val_metric < self.best_val_metric))
                if is_better:
                    self.best_val_metric = current_val_metric
                    logger_trainer_phase1.info(f"New best P1 val metric ({self.best_val_metric_name}): {self.best_val_metric:.4f}.")
                    self._save_checkpoint(is_best=True, reason=f"best_val_p1_{self.best_val_metric_name.replace('val_p1_','')}")
            if self.save_interval == 0:
                 self._save_checkpoint(is_best=False, reason="end_of_epoch_p1")
...
def parse_arguments_phase1() -> argparse.Namespace:
    ...
    parser.add_argument("--lambda_vsp", type=float, default=0.0)
    ...
    parser.add_argument("--checkpoint_dir", type=str, default="etp_phase1_reconstruction/checkpoints_phase1_rec")
    parser.add_argument("--load_checkpoint", type=str, default=None)
    parser.add_argument("--log_interval", type=int, default=20)
    parser.add_argument("--save_interval", type=int, default=500)
    parser.add_argument("--val_interval_epochs", type=int, default=1)
    parser.add_argument("--wandb_project", type=str, default="ETP_Phase1_Reconstruction")
    ...
    for json_arg_name_suffix in ['optimizer_kwargs_wubu_core', 'optimizer_kwargs_mlps']:
        ...
        try:
            setattr(args, json_arg_name_suffix, json.loads(json_str_val))
...
def main_phase1():
    ...
    logger_run_phase1.info(f"Device for Phase 1: {run_device}")
    ...
    if args.wubu_core_config_json:
        try:
            with open(args.wubu_core_config_json, 'r') as f_wubu:
                effective_wubu_core_config.update(json.load(f_wubu))
            logger_run_phase1.info(f"Loaded custom WuBu core config for Phase 1 from: {args.wubu_core_config_json}")
        except Exception as e_wubu_json:
            logger_run_phase1.warning(f"Error with WuBu JSON {args.wubu_core_config_json}: {e_wubu_json}. Using default.")
    ...
    for qc_name_key in ["sphere_wubu_core", "sphere_mlps"]:
        ...
        if json_path_val:
            try:
                with open(json_path_val, 'r') as f_qc:
                    q_controller_configs[qc_name_key] = json.load(f_qc)
                logger_run_phase1.info(f"Loaded Q-Controller config for {qc_name_key} (Phase 1) from: {json_path_val}")
            except Exception as e_qc_json:
                logger_run_phase1.warning(f"Error with Q-Ctrl JSON for {qc_name_key} ({json_path_val}): {e_qc_json}. Defaults used.")
                q_controller_configs[qc_name_key] = None
        else:
            q_controller_configs[qc_name_key] = None

    logger_run_phase1.info("Instantiating Dataset and DataLoader for Phase 1...")
    dataset_instance = DeepSeekR1EmbeddingDataset_P1(args.embeddings_file_A, args.embeddings_file_B)
