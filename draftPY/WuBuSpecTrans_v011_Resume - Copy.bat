...
REM --- Automatically Find Checkpoint to Load ---
SET "LOAD_CHECKPOINT="
...
IF EXIST "%CHECKPOINT_OUTPUT_DIR%\%BEST_CKPT_NAME%" (
    ...
    ECHO Found best checkpoint: !LOAD_CHECKPOINT!
) ELSE (
    ...
    IF DEFINED LATEST_EPOCH_CKPT_NAME (
        ...
        ECHO Found latest epoch checkpoint: !LOAD_CHECKPOINT!
    ) ELSE (
        ECHO No suitable checkpoint found in %CHECKPOINT_OUTPUT_DIR% to resume from. Starting new run.
    )
)


REM --- Training Configuration (Based on your last provided settings) ---
...
REM --- New: Flag for Q-Controller Reset on Load ---
SET "RESET_Q_CONTROLLERS_ON_LOAD=true"

REM --- New: Heuristic Discriminator Switching Parameters ---
SET "ENABLE_HEURISTIC_DISC_SWITCHING=true"
SET "INITIAL_DISC_TYPE=mel" 
SET "DISC_SWITCH_CHECK_INTERVAL=20"
SET "DISC_SWITCH_MIN_STEPS_BETWEEN=10"
SET "DISC_SWITCH_PROBLEM_STATE_COUNT_THRESH=4"


REM --- Construct Script Arguments ---
SET "SCRIPT_ARGS="
...
IF DEFINED LOAD_CHECKPOINT (
    ...
    ECHO WARNING: No checkpoint specified to load for resume. Starting potentially new run.
)
...
ECHO ======================================================
ECHO RESUMING WuBuSpecTrans VAE-GAN - Audio Run
ECHO Python: %PYTHON_EXE%
ECHO Script Name: %SCRIPT_NAME%
ECHO Checkpoints From: %CHECKPOINT_OUTPUT_DIR%
ECHO Loading Checkpoint: %LOAD_CHECKPOINT%
ECHO Audio Data: %AUDIO_DATA_PATH%
ECHO Primary Discriminator Input Type: %DISCRIMINATOR_INPUT_TYPE%
ECHO Enable Heuristic D Switching: %ENABLE_HEURISTIC_DISC_SWITCHING%
...
ECHO Starting RESUMED training script: %SCRIPT_NAME%
ECHO.
ECHO Full command:
...
IF %NPROC_PER_NODE% EQU 1 (
    ...
    "%PYTHON_EXE%" -m torch.distributed.run --nproc_per_node=%NPROC_PER_NODE% --master_addr=localhost --master_port=29515 --standalone --nnodes=1 "%FULL_SCRIPT_PATH%" !SCRIPT_ARGS!
)
