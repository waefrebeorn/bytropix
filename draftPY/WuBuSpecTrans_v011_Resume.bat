...
SET "LOAD_CHECKPOINT_VALUE=C:\projects\bytropix\draftPY\..\checkpoints\WuBuSpecTrans_v011_Run_MelReconFocus_VRAMPush\wubuspectrans_ckpt_v011_ep1_step326.pt"
...
SET "LEARNING_RATE_GEN=1e-5"
SET "LEARNING_RATE_DISC=5e-5"
SET "LEARNING_RATE_DISC_ALT=5e-5"
SET "RISGD_MAX_GRAD_NORM=3.5"
SET "GLOBAL_MAX_GRAD_NORM=5.5"
...
SET "DISC_TARGET_FINAL_FEATURE_DIM_H=8"
SET "DISC_TARGET_FINAL_FEATURE_DIM_W=8"
SET "MAX_MEL_DISC_DOWNSAMPLE_LAYERS=6"
SET "USE_MEL_D_ATTENTION=true"
SET "MEL_D_ATTENTION_IDX=3"
SET "MEL_D_MSD_NUM_SCALES=3"
SET "MEL_D_MSD_SHARE_WEIGHTS=false"

SET "DISC_DCT_EMBED_DIM=128"
SET "DISC_DCT_USE_POS_EMBED=true"
SET "DISC_DCT_USE_CLS_TOKEN=true"
SET "DISC_TRANSFORMER_NHEAD=8"
SET "DISC_TRANSFORMER_DIM_FEEDFORWARD=1024"
SET "DISC_TRANSFORMER_DROPOUT=0.1"
SET "DISC_TRANSFORMER_NUM_LAYERS=4"
SET "DISC_TRANSFORMER_NORM_FIRST=true"

SET "DISC_USE_GLOBAL_STATS_AUX=true"
SET "DISC_GLOBAL_STATS_MLP_HIDDEN_DIM=64"
...
SET "WUBU_G_USE_ROTATION=true"
SET "WUBU_G_PHI_CURVATURE=true"
SET "WUBU_G_PHI_ROT_INIT=true"

SET "WUBU_D_NUM_LEVELS=4"
SET "WUBU_D_HYPERBOLIC_DIMS=384 256 192 128"
SET "WUBU_D_INITIAL_CURVATURES=0.9 0.7 0.6 0.5"
SET "WUBU_D_USE_ROTATION=true"
SET "WUBU_D_PHI_CURVATURE=true"
SET "WUBU_D_PHI_ROT_INIT=true"
SET "WUBU_D_OUTPUT_DIM=128"

SET "WUBU_D_REGION_NUM_LEVELS=2"
SET "WUBU_D_REGION_FEATURE_DIM=128"
SET "WUBU_D_REGION_HYPERBOLIC_DIMS=128 128"
SET "WUBU_D_REGION_INITIAL_CURVATURES=0.6 0.4"
SET "WUBU_D_REGION_USE_ROTATION=false"
SET "WUBU_D_REGION_PHI_CURVATURE=false"
SET "WUBU_D_REGION_PHI_ROT_INIT=false"

SET "WUBU_D_ALT_NUM_LEVELS=2"
SET "WUBU_D_ALT_HYPERBOLIC_DIMS=128 64"
SET "WUBU_D_ALT_INITIAL_CURVATURES=0.5 0.3"
SET "WUBU_D_ALT_USE_ROTATION=false"
SET "WUBU_D_ALT_PHI_CURVATURE=false"
SET "WUBU_D_ALT_PHI_ROT_INIT=false"
SET "WUBU_D_ALT_OUTPUT_DIM=64"
...
SET "MIN_LAMBDA_KL_Q_CONTROL=1e-4"
SET "MAX_LAMBDA_KL_Q_CONTROL=0.01"
SET "Q_LKL_SCALE_OPTIONS=0.75 0.90 1.0 1.10 1.25"
SET "Q_LKL_LR_MOM_PROBATION_STEPS=25"
SET "Q_LKL_ACTION_PROBATION_STEPS=25"
...
SET "HEURISTIC_CHECK_INTERVAL=15"
SET "HEURISTIC_SHORT_TERM_HISTORY_LEN=5"
SET "HEURISTIC_TRIGGER_COUNT_THRESH=3"

SET "DISC_SWITCH_CHECK_INTERVAL=50"
...
SET "HEURISTIC_KL_HIGH_THRESH=10.0"
SET "HEURISTIC_RECON_STAGNATION_IMPROVEMENT_THRESH_REL=0.003"
...
SET "G_EASY_WIN_PENALTY_EPS_DENOM=1e-4"
SET "MAX_G_EASY_WIN_PENALTY_ABS=5.0"
...
SET "HEURISTIC_MIN_LAMBDA_GAN_FACTOR=0.7"
SET "HEURISTIC_MAX_LAMBDA_GAN_FACTOR=1.3"
...
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_target_final_feature_dim %DISC_TARGET_FINAL_FEATURE_DIM_H% %DISC_TARGET_FINAL_FEATURE_DIM_W%"
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --max_mel_disc_downsample_layers %MAX_MEL_DISC_DOWNSAMPLE_LAYERS%"
IF /I "!USE_MEL_D_ATTENTION!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --use_mel_d_attention"
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --mel_d_attention_idx %MEL_D_ATTENTION_IDX%"
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --mel_d_msd_num_scales %MEL_D_MSD_NUM_SCALES%"
IF /I "!MEL_D_MSD_SHARE_WEIGHTS!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --mel_d_msd_share_weights"

IF DEFINED DISC_DCT_EMBED_DIM ( IF NOT "!DISC_DCT_EMBED_DIM!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_dct_embed_dim %DISC_DCT_EMBED_DIM%" )
IF /I "!DISC_DCT_USE_POS_EMBED!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_dct_use_pos_embed"
IF /I "!DISC_DCT_USE_CLS_TOKEN!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_dct_use_cls_token"
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_transformer_nhead %DISC_TRANSFORMER_NHEAD%"
IF DEFINED DISC_TRANSFORMER_DIM_FEEDFORWARD ( IF NOT "!DISC_TRANSFORMER_DIM_FEEDFORWARD!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_transformer_dim_feedforward %DISC_TRANSFORMER_DIM_FEEDFORWARD%" )
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_transformer_dropout %DISC_TRANSFORMER_DROPOUT%"
SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_transformer_num_layers %DISC_TRANSFORMER_NUM_LAYERS%"
IF /I "!DISC_TRANSFORMER_NORM_FIRST!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_transformer_norm_first"
IF /I "!DISC_USE_GLOBAL_STATS_AUX!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_use_global_stats_aux"
IF DEFINED DISC_GLOBAL_STATS_MLP_HIDDEN_DIM ( IF NOT "!DISC_GLOBAL_STATS_MLP_HIDDEN_DIM!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --disc_global_stats_mlp_hidden_dim %DISC_GLOBAL_STATS_MLP_HIDDEN_DIM%" )
...
IF DEFINED WUBU_D_REGION_HYPERBOLIC_DIMS ( IF NOT "!WUBU_D_REGION_HYPERBOLIC_DIMS!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_region_hyperbolic_dims %WUBU_D_REGION_HYPERBOLIC_DIMS%" )
IF DEFINED WUBU_D_REGION_INITIAL_CURVATURES ( IF NOT "!WUBU_D_REGION_INITIAL_CURVATURES!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_region_initial_curvatures %WUBU_D_REGION_INITIAL_CURVATURES%" )
IF /I "!WUBU_D_REGION_USE_ROTATION!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_region_use_rotation"
IF /I "!WUBU_D_REGION_PHI_CURVATURE!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_region_phi_influence_curvature"
IF /I "!WUBU_D_REGION_PHI_ROT_INIT!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_region_phi_influence_rotation_init"


IF DEFINED WUBU_D_ALT_NUM_LEVELS ( IF NOT "!WUBU_D_ALT_NUM_LEVELS!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_num_levels %WUBU_D_ALT_NUM_LEVELS%" )
IF DEFINED WUBU_D_ALT_HYPERBOLIC_DIMS ( IF NOT "!WUBU_D_ALT_HYPERBOLIC_DIMS!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_hyperbolic_dims %WUBU_D_ALT_HYPERBOLIC_DIMS%" )
IF DEFINED WUBU_D_ALT_INITIAL_CURVATURES ( IF NOT "!WUBU_D_ALT_INITIAL_CURVATURES!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_initial_curvatures %WUBU_D_ALT_INITIAL_CURVATURES%" )
IF /I "!WUBU_D_ALT_USE_ROTATION!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_use_rotation"
IF /I "!WUBU_D_ALT_PHI_CURVATURE!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_phi_influence_curvature"
IF /I "!WUBU_D_ALT_PHI_ROT_INIT!"=="true" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_phi_influence_rotation_init"
IF DEFINED WUBU_D_ALT_OUTPUT_DIM ( IF NOT "!WUBU_D_ALT_OUTPUT_DIM!"=="" SET "SCRIPT_ARGS=!SCRIPT_ARGS! --wubu_d_alt_output_dim %WUBU_D_ALT_OUTPUT_DIM%" )
