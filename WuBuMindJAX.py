# Wubu: Final fix. Training was successful. The generation crash was a classic
# JAX error: trying to JIT-compile a function that uses Python-native objects
# (strings, dicts). The fix is to separate concerns:
# 1. A small, JIT-compiled `predict_step` function for pure numerical work.
# 2. A standard Python `generate` loop to handle strings and call `predict_step`.
# This is a robust and standard JAX pattern. The model is complete.
...
class WubuMind(nn.Module):
    context_length: int; vocab_size: int; d_model: int; n_heads: int; n_layers: int; k_neighbors: int; modulus: int; poincare_c: float = 1.0
    @nn.compact
    def __call__(self, hashes, indices):
        B = hashes.shape[0]
        char_embed = nn.Embed(self.vocab_size, self.d_model, name="token_embedding")(indices)
        ...
        x = nn.Dense(self.d_model, name="bridge_proj")(jnp.concatenate([char_embed, hash_embed], axis=-1))
...
# --- JIT-compiled function for a single prediction step ---
@partial(jax.jit, static_argnames=("model", "temperature"))
def predict_step(state, model, hashes, indices, key, temperature):
    logits = model.apply({'params': state.params}, hashes, indices)[0]
    logits /= temperature
    next_idx = jax.random.categorical(key, logits)
    return next_idx

# --- Python-level orchestrator for generation (NOT JIT-compiled) ---
def generate(state, model, ascii_converter, hasher, key, prompt, steps, temperature=0.6):
    ...
    if len(indices) < min_len:
        pad_len = min_len - len(indices)
        pad_char_idx = ascii_converter.char_to_idx.get(' ', 0)
        pad_char_val = ascii_converter.char_to_val.get(' ', 0)
    ...
    for _ in tqdm(range(steps), desc="Generating"):
        ...
        context_values = values[-(min_len):]
        context_indices = indices[-model.context_length:]
        context_hashes_arr = jnp.array(hasher.hash_sequence(context_values))[None, :]
        context_indices_arr = jnp.array(context_indices)[None, :]

        next_idx = predict_step(state, model, context_hashes_arr, context_indices_arr, step_key, temperature)
        next_idx_item = next_idx.item()

        next_char = ascii_converter.idx_to_char.get(next_idx_item, ' ')
        next_val = ascii_converter.char_to_val.get(next_char, 0)
...
def train_step(state, batch):
    hashes, indices, targets = batch
    def loss_fn(params):
        logits = state.apply_fn({'params': params}, hashes, indices)
...
def main():
    ...
    MODULUS, MODEL_FILE = 10**9+7, "wubumind_flax_v3.pkl"
    FORCE_RETRAIN = True # <-- THE FIX IS HERE
    ...
    if os.path.exists(MODEL_FILE) and not FORCE_RETRAIN:
        ...
        h, ind, t = [], [], []; num_examples = len(indices) - CONTEXT_LENGTH - HASH_WINDOW
        ...
        all_hashes, all_indices, all_targets = jnp.array(h), jnp.array(ind), jnp.array(t)

        key, init_key = jax.random.split(key)
        # We need to use model.apply here to correctly pass vocab_size
        params = model.init(init_key, all_hashes[:1], all_indices[:1])['params']
        tx = optax.chain(optax.clip(1.0), optax.adamw(LEARNING_RATE, weight_decay=0.01))
        state = train_state.TrainState.create(apply_fn=model.apply, params=params, tx=tx)
        ...
        for epoch in range(EPOCHS):
            ...
            for i in tqdm(range(num_batches), desc=f"Epoch {epoch+1}/{EPOCHS}"):
                ...
                batch = (all_hashes[batch_idx], all_indices[batch_idx], all_targets[batch_idx])
