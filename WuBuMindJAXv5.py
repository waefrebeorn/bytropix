...
def train_step(state, batch, static_apply_fn, force_nan: bool):
    ...
    reported_loss = jnp.where(is_nan, 5.0, loss)
            ...
            for epoch in range(start_epoch, self.config['epochs']):
                ...
                with tqdm(range(steps_per_epoch), desc=f"Epoch {epoch+1}/{self.config['epochs']}") as pbar:
                    for step_idx in pbar:
                        ...
                        if is_nan:
                            tqdm.write(f"\n[!] WARNING: NaN loss detected at epoch {epoch+1}, step {step_idx+1}. Skipping gradient update. Loss set to 5.0.")
...
def training_main():
    ...
    TRAINING_CONFIG = {'epochs':20, 'batch_size':8, 'peak_learning_rate':5e-4, 'warmup_steps':500, 'force_retrain':False, 'context_length': 256, 'TEST_NAN_HANDLING': False, 'TEST_NAN_HANDLING_STEP': 5}
...
class WebRadioManager:
    def __init__(self, model_basename):
        ...
        lr_schedule = optax.constant_schedule(1e-5) # Use a small, constant learning rate for fine-tuning
