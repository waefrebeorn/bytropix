...
class TransformerBlock(nn.Module):
    """
    [DEBUGGING VERSION] Transformer block with ALL remat/checkpointing
    temporarily disabled to isolate tracer errors.
    """
...
class GenerativeConductor(nn.Module):
    ...
    # [THE DEFINITIVE FIX]
    # Define special token IDs as class attributes, making them accessible
    # from outside the model instance (e.g., self.model.MASK_TOKEN_ID).
    # We compute them here based on the static `num_codes` attribute.
    @property
    ...
    def setup(self):
        # The definitions are moved out, we just use them here.
        ...
        self.to_clip_embedding = nn.Dense(self.clip_dim, name='to_clip_embedding', dtype=self.dtype)
    ...
    def _forward_pass(self, tokens, text_emb, train: bool = False):
        B, L = tokens.shape
        if train:
        ...
        for block in self.blocks:
            x = block(x, ctx, mask=None, decode=False)
            
        final_hidden_state = self.norm(x)
        return final_hidden_state

    def __call__(self, target_tokens, text_emb, train: bool = True):
        B, L = target_tokens.shape
        
        key = self.make_rng('dropout')
        rand = jax.random.uniform(key, (B, L))
        num_masked = int(self.mask_ratio * L)
        should_mask = jnp.argsort(rand, axis=-1) < num_masked
        
        input_tokens = jnp.where(should_mask, self.MASK_TOKEN_ID, target_tokens)

        final_hidden_state = self._forward_pass(input_tokens, text_emb, train=train)
        
        logits = self.logit_head(final_hidden_state)
        
        if self.is_initializing():
            _ = self.to_clip_embedding(final_hidden_state)
    ...
    def get_features(self, tokens, text_emb, train: bool = False):
        final_hidden_state = self._forward_pass(tokens, text_emb, train)
        logits = self.logit_head(final_hidden_state)
        projected_features = self.to_clip_embedding(final_hidden_state)
...
class ChimeraLayoutManager:
    """
    [UPGRADED] Handles the creation and updating of the Rich TUI for the Chimera
    training ecosystem. Visualizes the state of all specialized clones, the new
    hyperparameter-tuning agents, and the live step profiler.
    """
    ...
    def generate_layout(self) -> Layout:
        """Creates the dynamic Rich Layout based on the Chimera trainer's current state."""
        trainer = self.trainer
        
        with trainer.ui_lock:
            # --- Header (Unchanged) ---
            trainer.spinner_idx = (trainer.spinner_idx + 1) % len(trainer.spinner_chars)
            spinner = trainer.spinner_chars[trainer.spinner_idx]
            precision_str = "[bold purple]BF16[/]" if trainer.dtype == jnp.bfloat16 else "[dim]FP32[/]"
            header_text = f"🧬 [bold]CHIMERA CONDUCTOR[/] | Master Params: [yellow]{trainer.param_count/1e6:.2f}M[/] | Precision: {precision_str}"
            header_panel = Panel(Align.center(header_text), style="bold red", title="[dim]wubumind.ai[/dim]", title_align="right", subtitle="[bold]A Sentient Training Ecosystem[/bold]")

            # --- Main Content Area ---
            main_layout = Layout(name="main")
            main_layout.split_row(
                Layout(name="clones_left", ratio=1),
                Layout(name="clones_right", ratio=1),
                Layout(name="preview", ratio=1)
            )

            # --- Clone Panels (With MMD Lambda Display) ---
            clone_panels = []
            for i in range(trainer.num_clones):
                spec = trainer.clone_specializations[i]
                metrics = trainer.last_metrics_for_ui[i]
                agent = trainer.clone_agents[i]
                
                if spec == "Scribe": color, icon = "cyan", "✍️"
                elif spec == "Structuralist": color, icon = "magenta", "🕸️"
                elif spec == "Purist": color, icon = "yellow", "💧"
                elif spec == "Visionary": color, icon = "green", "👁️"
                else: color, icon = "white", "❓"
                
                tbl = Table.grid(expand=True, padding=(0,1))
                tbl.add_column(style="dim", width=10); tbl.add_column()
                
                lr_agent = agent['lr_controller']
                status_short = lr_agent.status.split(' ')[0]
                ...
                tbl.add_row("Status", f"[{s_color}]{status_short}[/] {s_emoji}")
                tbl.add_row("LR", f"{lr_agent.current_value:.2e}")

                if spec == "Scribe": tbl.add_row("CE Loss", f"{metrics.get('Scribe/CE_Loss', 0):.4f}")
                elif spec == "Structuralist":
                    tbl.add_row("CE Loss", f"{metrics.get('Structuralist/CE_Loss', 0):.4f}")
                    tbl.add_row("MMD Loss", f"{metrics.get('Structuralist/MMD_Loss', 0):.4f}")
                    # [NEW] Display the dynamic lambda_mmd from its controller
                    if 'mmd_controller' in agent:
                        tbl.add_row("λ MMD", f"[yellow]{agent['mmd_controller'].current_value:.3f}[/]")
                elif spec == "Purist": tbl.add_row("Uncond CE", f"{metrics.get('Purist/Uncond_CE_Loss', 0):.4f}")
                elif spec == "Visionary":
                    tbl.add_row("CE Loss", f"{metrics.get('Visionary/CE_Loss', 0):.4f}")
                    tbl.add_row("CLIP Loss", f"{metrics.get('Visionary/CLIP_Loss', 0):.4f}")

                clone_panels.append(Panel(tbl, title=f"[bold {color}]{icon} {spec}[/]", border_style=color))

            main_layout["clones_left"].split_column(clone_panels[0], clone_panels[1])
            main_layout["clones_right"].split_column(clone_panels[2], clone_panels[3])
            
            # --- Right Column (System Stats, Preview, NEW Global Agents) ---
            preview_right_layout = Layout(name="preview_right")
            ...
            # [NEW] Global Agents Panel
            global_agents_tbl = Table.grid(expand=True, padding=(0,1)); global_agents_tbl.add_column(style="dim", width=11); global_agents_tbl.add_column()
            mask_agent = trainer.global_hp_controller
            mask_status_short = mask_agent.status.split(' ')[0]
            if "IMPROVING" in mask_status_short: m_emoji, m_color = "😎", "green"
            elif "STAGNATED" in mask_status_short: m_emoji, m_color = "🤔", "yellow"
            else: m_emoji, m_color = "😠", "red"
            global_agents_tbl.add_row("Status (Val)", f"[{m_color}]{mask_status_short}[/] {m_emoji}")
            global_agents_tbl.add_row("Mask Ratio", f"[yellow]{mask_agent.current_value:.3f}[/]")
            global_agents_panel = Panel(global_agents_tbl, title="[bold]🌍 Global Agents[/]", border_style="yellow", height=4)
            ...
            preview_content = trainer.rendered_preview if trainer.rendered_preview else Align.center("...Awaiting Validation...")
            preview_group = Group(prompt_text, preview_content)
            preview_panel = Panel(preview_group, title="[bold]🖼️ Master Model Preview[/]", border_style="green")
            
            preview_right_layout.split_column(global_stats_panel, global_agents_panel, preview_panel)
            main_layout["preview"].update(preview_right_layout)

            # --- Profiler Panel (Unchanged) ---
            profiler_table = Table("Stage", "Time (ms)", "Percent", title_style="bold yellow", box=None)
            profiler_table.add_column(style="dim"); profiler_table.add_column(justify="right"); profiler_table.add_column(justify="right")
            total_time = sum(trainer.profiler_data.values())
            if total_time > 0:
                for stage, time_ms in trainer.profiler_data.items():
                    percent = (time_ms / total_time) * 100
                    profiler_table.add_row(stage, f"{time_ms:.2f} ms", f"{percent:.1f}%")
                profiler_table.add_row("[bold]Total Step Time[/bold]", f"[bold]{total_time:.2f} ms[/bold]", "100.0%")
            status_text = "[bold green]ACTIVE[/]" if trainer.profiler_active else "[dim]INACTIVE[/]"
            profiler_panel = Panel(profiler_table, title=f"⏱️  [bold]Live Step Profiler[/] - {status_text}", subtitle="[dim](Press 'p' to toggle)[/dim]", border_style="yellow")
            ...
            root_layout.split_column(
                ...
                Layout(profiler_panel, name="profiler", size=8),
...
class BaseConductorTrainer(AdvancedTrainer):
    ...
    def __init__(self, args):
        ...
        self.cond_config = {
            ...
            'num_positions': self.token_map_size,
    ...
    def _get_common_train_setup(self):
        ...
        train_ds = tf.data.Dataset.from_tensor_slices((train_tokens, train_embeddings)).shuffle(10000, seed=self.args.seed).repeat().batch(self.args.batch_size, drop_remainder=True).prefetch(tf.data.AUTOTUNE)
        train_iterator = train_ds.as_numpy_iterator()

        val_ds = None
        if len(val_tokens) > 0:
             val_ds = tf.data.Dataset.from_tensor_slices((val_tokens, val_embeddings)).batch(self.args.batch_size, drop_remainder=True).prefetch(tf.data.AUTOTUNE)
             console.print(f"✅ Validation dataset prepared with {len(val_tokens)} samples (streaming).")

        return key_listener_thread, preview_executor, train_iterator, val_ds
        
    def train(self):
        raise NotImplementedError("Trainer subclasses must implement the `train` method.")
        
        
class ChimeraConductorTrainer(BaseConductorTrainer):
    def __init__(self, args):
        super().__init__(args)
        self.console = Console()
        self.console.print("--- [MODE] Sentient Hyperparameter Co-evolution is ACTIVE. ---", style="bold yellow")
        
        self.num_clones = 4
        self.clone_specializations = ["Scribe", "Structuralist", "Purist", "Visionary"]
        self.master_state: Optional[CustomTrainState] = None
        self.clone_states: List[CustomTrainState] = []
        self.clone_agents: List[Dict[str, Any]] = []

        # --- LR Q-Controller Config (remains the same) ---
        ...
        # --- NEW: Lambda_MMD Q-Controller Config ---
        q_mmd_config = {"q_table_size":50,"num_actions":5,"action_factors":[0.9,0.98,1.0,1.02,1.1],"learning_rate_q":0.1,"discount_factor_q":0.9,"value_min":0.1,"value_max":5.0,"metric_history_len":2000,"metric_min":0.001,"metric_max":0.1,"exploration_rate_q":0.2,"min_exploration_rate":0.05,"exploration_decay":0.9998,"trend_window":300,"improve_threshold":1e-5,"regress_threshold":1e-6,"regress_penalty":5.0,"stagnation_penalty":-1.0,"warmup_steps":300,"warmup_start_val":args.lambda_mmd}

        # --- NEW: Mask_Ratio Q-Controller Config ---
        q_mask_config = {"q_table_size":50,"num_actions":3,"action_factors":[0.99,1.0,1.01],"learning_rate_q":0.05,"discount_factor_q":0.95,"value_min":0.3,"value_max":0.75,"metric_history_len":2000,"metric_min":0.1,"metric_max":8.0,"exploration_rate_q":0.25,"min_exploration_rate":0.05,"exploration_decay":0.9999,"trend_window":500,"improve_threshold":1e-4,"regress_threshold":1e-5,"regress_penalty":8.0,"stagnation_penalty":-1.5,"warmup_steps":500,"warmup_start_val":args.mask_ratio}
        
        # Instantiate all controllers
        for spec in self.clone_specializations:
            agent_dict = {'lr_controller': JaxHakmemQController(initial_value=args.lr, config=q_lr_config, param_name="LR")}
            if spec == "Structuralist":
                agent_dict['mmd_controller'] = JaxHakmemQController(initial_value=args.lambda_mmd, config=q_mmd_config, param_name="λ_mmd")
            self.clone_agents.append(agent_dict)
        
        self.global_hp_controller = JaxHakmemQController(initial_value=args.mask_ratio, config=q_mask_config, param_name="Mask-Ratio")

        self.ui_lock = threading.Lock()
        self.last_metrics_for_ui = [{} for _ in range(self.num_clones)]
        self.layout_manager = ChimeraLayoutManager(self)
        self._global_step_fn = self._compile_global_step()
        self.profiler_active = False
        self.profiler_data = {}

    # ... (_re_clone_cpu, _initialize_states, _compile_global_step methods are unchanged) ...
    def _re_clone_cpu(self):
        master_params = self.master_state.params
        self.clone_states = [cs.replace(params=master_params) for cs in self.clone_states]
    def _initialize_states(self):
        ...
        def get_initial_variables(init_key):
            dummy_tokens = jnp.zeros((1, self.token_map_size), jnp.int32)
            dummy_embeddings = jnp.zeros((1, 512), self.dtype)
            return self.model.init({'params': init_key, 'dropout': init_key}, dummy_tokens, dummy_embeddings, train=True)
        with jax.default_device(CPU_DEVICE):
            params = get_initial_variables(key)['params']
            self.master_state = CustomTrainState.create(apply_fn=self.model.apply, params=params, tx=optimizer)
            self.clone_states = [CustomTrainState.create(apply_fn=self.model.apply, params=params, tx=optimizer) for _ in range(self.num_clones)]
    def _compile_global_step(self):
        class CloneArgs(NamedTuple):
            state: TrainState; batch: Tuple[jnp.ndarray, jnp.ndarray]; dropout_key: jnp.ndarray; codebook: jnp.ndarray; lambdas: jnp.ndarray
        def _get_varentropy_fitness(logits):
            logits_flat=logits.reshape(-1,logits.shape[-1]);log_probs=jax.nn.log_softmax(jax.lax.stop_gradient(logits_flat));p=jnp.exp(log_probs);ent=-jnp.sum(p*log_probs,axis=-1);diff=log_probs+ent[...,None];varent=jnp.sum(p*diff**2,axis=-1);return jnp.mean(varent)
        def _scribe_loss_fn(params,args:CloneArgs):
            tokens_flat,embeddings=args.batch;ce_loss,all_logits=args.state.apply_fn({'params':params},tokens_flat,embeddings,train=True,rngs={'dropout':args.dropout_key});metrics={'Scribe/CE_Loss':ce_loss.astype(self.dtype),'Structuralist/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Structuralist/MMD_Loss':jnp.array(0.0,dtype=self.dtype),'Purist/Uncond_CE_Loss':jnp.array(0.0,dtype=self.dtype),'Visionary/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Visionary/CLIP_Loss':jnp.array(0.0,dtype=self.dtype),};fitness=_get_varentropy_fitness(all_logits);return (ce_loss.astype(self.dtype),metrics,fitness)
        def _structuralist_loss_fn(params,args:CloneArgs):
            lambda_ce,lambda_mmd=args.lambdas[0],args.lambdas[1];tokens_flat,embeddings=args.batch;ce_loss,all_logits=args.state.apply_fn({'params':params},tokens_flat,embeddings,train=True,rngs={'dropout':args.dropout_key});probs=nn.softmax(all_logits,axis=-1);fake_samples=jnp.einsum('blv,vd->bld',probs,args.codebook);real_samples=args.codebook[tokens_flat];mmd_loss=calculate_mmd_loss(jax.lax.stop_gradient(real_samples),fake_samples);total_loss=(lambda_ce*ce_loss)+(lambda_mmd*mmd_loss);metrics={'Scribe/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Structuralist/CE_Loss':ce_loss.astype(self.dtype),'Structuralist/MMD_Loss':mmd_loss.astype(self.dtype),'Purist/Uncond_CE_Loss':jnp.array(0.0,dtype=self.dtype),'Visionary/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Visionary/CLIP_Loss':jnp.array(0.0,dtype=self.dtype),};fitness=_get_varentropy_fitness(all_logits);return (total_loss.astype(self.dtype),metrics,fitness)
        def _purist_loss_fn(params,args:CloneArgs):
            tokens_flat,_=args.batch;uncond_embeddings=jnp.repeat(params['uncond_embedding'],tokens_flat.shape[0],axis=0);ce_loss,all_logits=args.state.apply_fn({'params':params},tokens_flat,uncond_embeddings,train=True,rngs={'dropout':args.dropout_key});metrics={'Scribe/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Structuralist/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Structuralist/MMD_Loss':jnp.array(0.0,dtype=self.dtype),'Purist/Uncond_CE_Loss':ce_loss.astype(self.dtype),'Visionary/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Visionary/CLIP_Loss':jnp.array(0.0,dtype=self.dtype),};fitness=_get_varentropy_fitness(all_logits);return (ce_loss.astype(self.dtype),metrics,fitness)
        def _visionary_loss_fn(params,args:CloneArgs):
            tokens_flat,embeddings=args.batch;output=args.state.apply_fn({'params':params},tokens_flat,embeddings,train=True,rngs={'dropout':args.dropout_key},method=self.model.get_features,mutable=False);all_logits,projected_features=output;ce_loss,_=args.state.apply_fn({'params':params},tokens_flat,embeddings,train=True,rngs={'dropout':args.dropout_key});seq_features=projected_features.mean(axis=1);seq_features_norm=seq_features/jnp.linalg.norm(seq_features,axis=-1,keepdims=True);text_features_norm=embeddings/jnp.linalg.norm(embeddings,axis=-1,keepdims=True);clip_loss=calculate_clip_similarity_loss(seq_features_norm,text_features_norm);total_loss=ce_loss+(0.1*clip_loss);metrics={'Scribe/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Structuralist/CE_Loss':jnp.array(0.0,dtype=self.dtype),'Structuralist/MMD_Loss':jnp.array(0.0,dtype=self.dtype),'Purist/Uncond_CE_Loss':jnp.array(0.0,dtype=self.dtype),'Visionary/CE_Loss':ce_loss.astype(self.dtype),'Visionary/CLIP_Loss':clip_loss.astype(self.dtype),};fitness=_get_varentropy_fitness(all_logits);return (total_loss.astype(self.dtype),metrics,fitness)
        _loss_branches=[_scribe_loss_fn,_structuralist_loss_fn,_purist_loss_fn,_visionary_loss_fn]
        @partial(jit,static_argnames=('num_codes','alpha'))
        def global_step_fn(master_state,stacked_clone_states,batches,dropout_keys,lrs,all_lambdas,num_codes,codebook,alpha):
            metric_keys=['Scribe/CE_Loss','Structuralist/CE_Loss','Structuralist/MMD_Loss','Purist/Uncond_CE_Loss','Visionary/CE_Loss','Visionary/CLIP_Loss'];initial_metrics={k:jnp.array(0.0,dtype=self.dtype)for k in metric_keys};stacked_initial_metrics=jax.tree_util.tree_map(lambda x:jnp.array([x]*self.num_clones),initial_metrics)
            def loop_body(i,carry):
                stacked_states,stacked_metrics,stacked_fitness=carry;current_clone_state=jax.tree_util.tree_map(lambda x:x[i],stacked_states);clone_args=CloneArgs(state=current_clone_state,batch=(batches[0][i],batches[1][i]),dropout_key=dropout_keys[i],codebook=codebook,lambdas=all_lambdas[i])
                def unified_loss_fn(params,args):
                    loss,metrics,fitness=jax.lax.switch(i,_loss_branches,params,args);return loss,(metrics,fitness)
                (loss,(metrics,fitness)),grads=jax.value_and_grad(unified_loss_fn,argnums=0,has_aux=True)(current_clone_state.params,clone_args);new_state=current_clone_state.apply_gradients(grads=grads,learning_rate=lrs[i]);new_stacked_states=jax.tree_util.tree_map(lambda stack,new_leaf:stack.at[i].set(new_leaf),stacked_states,new_state);new_stacked_metrics=jax.tree_util.tree_map(lambda stack,new_leaf:stack.at[i].set(new_leaf),stacked_metrics,metrics);new_stacked_fitness=stacked_fitness.at[i].set(fitness);return (new_stacked_states,new_stacked_metrics,new_stacked_fitness)
            stacked_fitness_placeholder=jnp.zeros(self.num_clones,dtype=self.dtype);init_carry=(stacked_clone_states,stacked_initial_metrics,stacked_fitness_placeholder);final_stacked_states,final_stacked_metrics,final_fitness_scores=jax.lax.fori_loop(0,self.num_clones,loop_body,init_carry);winner_index=jnp.argmin(final_fitness_scores);winning_params=jax.tree_util.tree_map(lambda x:x[winner_index],final_stacked_states.params);updated_master_params=jax.tree_util.tree_map(lambda master,winner:alpha*master+(1-alpha)*winner,master_state.params,winning_params);final_master_state=master_state.replace(params=updated_master_params,step=master_state.step+1);broadcast_master_params=jax.tree_util.tree_map(lambda x:jnp.repeat(x[None,...],self.num_clones,axis=0),updated_master_params);final_stacked_clone_states=final_stacked_states.replace(params=broadcast_master_params);return final_master_state,final_stacked_clone_states,final_stacked_metrics
        return global_step_fn

    def train(self):
        # ... (setup and compilation logic is unchanged) ...
        self.console.print("--- [PROJECT CHIMERA] Activating Sentient Conductor Training. ---", style="bold red")
        key = jax.random.PRNGKey(self.args.seed)
        tokenized_data_path = Path(self.args.data_dir) / f"tokenized_data_{self.args.basename}_{self.args.num_codes}c.npz"
        with np.load(tokenized_data_path) as data: train_samples = len(data['train_tokens'])
        key_listener_thread, preview_executor, train_iterator, val_ds = self._get_common_train_setup()
        active_preview_future = None
        checkpoint_executor = ThreadPoolExecutor(max_workers=1, thread_name_prefix='CheckpointSaver')
        self.console.print("⚡ [OBJECTIVE] Masked Image Modeling & Tournament Selection enabled.")
        steps_per_epoch = train_samples // self.args.batch_size
        self.console.print(f"--- 🧬 Chimera Dataset Info --- Training samples: {train_samples} Batch size: {self.args.batch_size} Global steps per epoch: {steps_per_epoch}")
        if not self.master_state: self._initialize_states()
        ckpt_path_final = Path(f"chimera_{self.args.basename}_{self.args.num_layers}l_final.pkl"); ckpt_path_best = Path(f"chimera_{self.args.basename}_{self.args.num_layers}l_best.pkl")
        best_val_loss, start_epoch, global_step = float('inf'), 0, 0
    
        if ckpt_path_final.exists():
            self.console.print(f"--- Resuming Chimera from checkpoint: [green]{ckpt_path_final}[/green] ---")
            with open(ckpt_path_final, 'rb') as f: ckpt = pickle.load(f)
            from flax.core import copy; fresh_params = self.master_state.params; loaded_params = ckpt['master_params']
            transferred_params = copy(fresh_params, loaded_params); self.master_state = self.master_state.replace(params=transferred_params)
            if 'clone_opt_states' in ckpt and len(ckpt['clone_opt_states']) == self.num_clones: self.clone_states = [cs.replace(opt_state=os) for cs, os in zip(self.clone_states, ckpt['clone_opt_states'])]
            self._re_clone_cpu()
            global_step = ckpt.get('global_step', 0); start_epoch = global_step // steps_per_epoch
            # [MODIFIED] Load state for all Q-Controllers
            for i in range(self.num_clones):
                if f'lr_controller_{i}' in ckpt: self.clone_agents[i]['lr_controller'].load_state_dict(ckpt[f'lr_controller_{i}'])
                if 'mmd_controller' in self.clone_agents[i] and 'mmd_controller_1' in ckpt: self.clone_agents[i]['mmd_controller'].load_state_dict(ckpt['mmd_controller_1'])
            if 'global_hp_controller' in ckpt: self.global_hp_controller.load_state_dict(ckpt['global_hp_controller'])
            if ckpt_path_best.exists():
                with open(ckpt_path_best, 'rb') as f_best: best_val_loss = pickle.load(f_best).get('val_loss', float('inf'))
            self.console.print(f"✅ Resumed from Epoch {start_epoch + 1}, Global Step {global_step}.")

        self.param_count = jax.tree_util.tree_reduce(lambda acc, x: acc + x.size, self.master_state.params, 0)
        p_master_state = jax.device_put(self.master_state)
        p_clone_states = jax.tree_util.tree_map(lambda *x: jnp.stack(x), *self.clone_states)
        p_codebook = jax.device_put(self.tokenizer_codebook)
        self.console.print("✅ All training states moved to GPU and stacked.")

        # --- Kernel Compilation ---
        def _compile_all_kernels():
            self.console.print("--- [JIT] Compiling global training step... ---")
            single_dummy_batch = next(train_iterator)
            dummy_batches = (jnp.stack([single_dummy_batch[0]] * self.num_clones), jnp.stack([single_dummy_batch[1]] * self.num_clones))
            dummy_batches_gpu = jax.device_put(dummy_batches)
            compile_key, *dummy_dropout_keys = jax.random.split(key, self.num_clones + 1)
            dummy_lrs = jnp.array([self.args.lr] * self.num_clones); dummy_lambdas = jnp.array([[1.0, self.args.lambda_mmd]] * self.num_clones)
            self._global_step_fn(p_master_state, p_clone_states, dummy_batches_gpu, jnp.stack(dummy_dropout_keys), dummy_lrs, dummy_lambdas, self.args.num_codes, p_codebook, 0.999)
            self.console.print("✅ Training kernel compiled.")
            
            @partial(jax.jit, static_argnames=('apply_fn',))
            def validation_loss_fn(params, val_tokens_batch, val_embeddings_batch, apply_fn, key):
                loss, _ = apply_fn({'params': params}, val_tokens_batch, val_embeddings_batch, train=False, rngs={'dropout': key})
                return loss

            if val_ds:
                dummy_val_batch_np = next(val_ds.as_numpy_iterator())
                dummy_val_batch_gpu = jax.device_put(dummy_val_batch_np)
                validation_loss_fn(p_master_state.params, dummy_val_batch_gpu[0], dummy_val_batch_gpu[1], self.model.apply, key)

            @partial(jax.jit, static_argnames=('model_apply_fn', 'num_steps', 'guidance_scale', 'resolution'))
            def generate_preview_jit(model_apply_fn, params, key, cond_emb, uncond_emb, guidance_scale, num_steps, resolution):
                B,L=1,self.token_map_size;MASK_ID=self.model.MASK_TOKEN_ID;t=jnp.linspace(0,1,num_steps+1);mask_schedule=jnp.cos(t*jnp.pi/2)
                def loop_body_v2(i,carry):
                    tokens,key=carry;stacked_tokens=jnp.concatenate([tokens,tokens],axis=0);stacked_emb=jnp.concatenate([cond_emb,uncond_emb],axis=0);logits_stacked=model_apply_fn({'params':params},stacked_tokens,stacked_emb,train=False,method=self.model.get_features)[0];logits_cond,logits_uncond=logits_stacked[0:1],logits_stacked[1:2];cfg_logits=logits_uncond+guidance_scale*(logits_cond-logits_uncond);key,sample_key,gumbel_key=jax.random.split(key,3);sampled_tokens=jax.random.categorical(sample_key,cfg_logits);unknown_mask=(tokens==MASK_ID);probs=jax.nn.softmax(cfg_logits,axis=-1);confidence=jnp.take_along_axis(probs,sampled_tokens[...,None],axis=-1).squeeze(-1);gumbel_noise=jax.random.gumbel(gumbel_key,confidence.shape);noisy_confidence=jnp.where(unknown_mask,confidence+gumbel_noise,-1e9);ranked_indices=jnp.argsort(noisy_confidence,axis=-1)[:,::-1];num_to_mask_next=jnp.floor(L*mask_schedule[i+1]).astype(jnp.int32);ranks=jnp.argsort(ranked_indices,axis=-1);num_revealed_next=L-num_to_mask_next;mask_for_updates=ranks<num_revealed_next;final_update_mask=unknown_mask&mask_for_updates;new_tokens=jnp.where(final_update_mask,sampled_tokens,tokens);return new_tokens,key
                initial_tokens=jnp.full((B,L),MASK_ID,dtype=jnp.int32);final_tokens,_=jax.lax.fori_loop(0,num_steps,loop_body_v2,(initial_tokens,key));grid_dim=self.tok_config['latent_grid_size']//4;token_grid=final_tokens.reshape(1,grid_dim,grid_dim);path_params=self.tokenizer.apply({'params':self.tok_params},token_grid,method=self.tokenizer.decode);coords=jnp.stack(jnp.meshgrid(jnp.linspace(-1,1,resolution),jnp.linspace(-1,1,resolution),indexing='ij'),-1).reshape(-1,2);coord_chunks=jnp.array_split(coords,(resolution**2)//(64**2));pixels_list=[self.p1_model.apply({'params':self.p1_params},path_params,c,method=self.p1_model.decode)for c in coord_chunks];img=jnp.concatenate(pixels_list,axis=1).reshape(1,resolution,resolution,3);return ((img*0.5+0.5)*255).astype(jnp.uint8)

            self.console.print("--- [JIT] Compiling validation & preview kernels... ---")
            dummy_emb = self.validation_embeddings[0:1]
            generate_preview_jit(self.model.apply, p_master_state.params, key, dummy_emb, dummy_emb, 4.0, 12, self.preview_resolution)
            self.console.print("✅ Validation & preview kernels compiled.")
            return validation_loss_fn, generate_preview_jit

        self.validation_loss_fn, self.generate_preview_fn = _compile_all_kernels()

        def _update_preview_task(master_params_cpu, uncond_embedding_cpu, current_prompt_idx, key):
            cond_emb = self.validation_embeddings[current_prompt_idx:current_prompt_idx+1]
            preview_img_array = self.generate_preview_fn(self.model.apply, master_params_cpu, key, cond_emb, uncond_embedding_cpu, 4.0, 12, self.preview_resolution)
            preview_img_array.block_until_ready()
            with self.ui_lock:
                if Pixels: self.rendered_preview = Align.center(Pixels.from_image(Image.fromarray(np.asarray(preview_img_array[0]))))
                else: self.rendered_preview = Align.center("[dim]Preview disabled[/dim]")

        self.progress = Progress(TextColumn("[bold]Epoch {task.completed}/{task.total}"), BarColumn(), TextColumn("Step {task.fields[step]}/{task.fields[steps_per_epoch]}"), "•", TextColumn("[bold green]Best Val: {task.fields[best_val_loss]:.4f}[/]"), TimeRemainingColumn())
        epoch_task = self.progress.add_task("train", total=self.args.epochs, completed=start_epoch, step=global_step % steps_per_epoch, steps_per_epoch=steps_per_epoch, best_val_loss=best_val_loss)

        last_eval_step = -1; active_save_future = None; last_step_time = time.time();
        p_states_for_saving = (p_master_state, p_clone_states); trigger_preview = True

        try:
            with Live(self.layout_manager.generate_layout(), screen=True, redirect_stderr=False, vertical_overflow="visible") as live:
                while global_step < (self.args.epochs * steps_per_epoch) and not (self.should_shutdown or self.interactive_state.shutdown_event.is_set()):
                    prompt_change = self.interactive_state.get_and_reset_preview_change()
                    if prompt_change != 0:
                        with self.ui_lock:
                            self.current_preview_prompt_idx = (self.current_preview_prompt_idx + prompt_change) % len(self.validation_prompts)
                            self.rendered_preview = None
                        trigger_preview = True
                    
                    if self.interactive_state.get_and_reset_profiler_toggle(): self.profiler_active = not self.profiler_active; self.profiler_data = {}
                    epoch = global_step // steps_per_epoch; step_in_epoch = global_step % steps_per_epoch
                    if step_in_epoch == 0 and global_step > 0: self.progress.update(epoch_task, completed=epoch)
                    if self.profiler_active: t0 = time.perf_counter()
                    
                    # [MODIFIED] Get actions from all Q-Controllers
                    self.model.mask_ratio = self.global_hp_controller.choose_action()
                    lrs = [agent['lr_controller'].choose_action() for agent in self.clone_agents]
                    all_lambdas = []
                    for i in range(self.num_clones):
                        lambda_ce, lambda_mmd = 1.0, 0.0
                        if self.clone_specializations[i] == "Structuralist":
                            lambda_mmd = self.clone_agents[i]['mmd_controller'].choose_action()
                        all_lambdas.append([lambda_ce, lambda_mmd])

                    single_batch_cpu = next(train_iterator); batches = (jnp.stack([single_batch_cpu[0]] * self.num_clones), jnp.stack([single_batch_cpu[1]] * self.num_clones))
                    key, *dropout_keys = jax.random.split(key, self.num_clones + 1)
                    if self.profiler_active: t1 = time.perf_counter(); self.profiler_data['1. CPU Data Prep'] = (t1 - t0) * 1000
                    p_master_state, p_clone_states, metrics_futures = self._global_step_fn(p_master_state, p_clone_states, jax.device_put(batches), jnp.stack(dropout_keys), jnp.array(lrs), jnp.array(all_lambdas), self.args.num_codes, p_codebook, 0.999)
                    final_stacked_metrics = jax.device_get(metrics_futures); p_states_for_saving = (p_master_state, p_clone_states)
                    if self.profiler_active: t2 = time.perf_counter(); self.profiler_data['2. JAX Global Step (Dispatch+Sync)'] = (t2 - t1) * 1000
                    
                    # [MODIFIED] Update all Q-Controllers
                    all_metrics_cpu = [{} for _ in range(self.num_clones)]
                    for i in range(self.num_clones):
                        full_metrics_cpu = {k: v[i].item() for k, v in final_stacked_metrics.items()}; relevant_metrics_cpu = {k: v for k, v in full_metrics_cpu.items() if v != 0.0}
                        all_metrics_cpu[i] = relevant_metrics_cpu
                        if relevant_metrics_cpu:
                            # Update LR controller with primary loss
                            self.clone_agents[i]['lr_controller'].update_q_value(list(relevant_metrics_cpu.values())[0])
                            # Update MMD controller with MMD loss if it exists
                            if 'mmd_controller' in self.clone_agents[i] and 'Structuralist/MMD_Loss' in relevant_metrics_cpu:
                                self.clone_agents[i]['mmd_controller'].update_q_value(relevant_metrics_cpu['Structuralist/MMD_Loss'])

                    if self.profiler_active: t3 = time.perf_counter(); self.profiler_data['3. CPU Post-Processing'] = (t3 - t2) * 1000
                    with self.ui_lock: self.last_metrics_for_ui = all_metrics_cpu
                    if self.profiler_active: t4 = time.perf_counter(); self.profiler_data['4. UI State Update'] = (t4 - t3) * 1000
                    
                    if (global_step - last_eval_step) >= self.args.eval_every:
                        trigger_preview = True; last_eval_step = global_step

                    if trigger_preview:
                        trigger_preview = False
                        if val_ds is not None and not (active_preview_future and not active_preview_future.done()):
                            master_params_cpu, uncond_emb_cpu = jax.device_get((p_master_state.params, p_master_state.params['uncond_embedding']))
                            key, preview_key, val_key = jax.random.split(key, 3)
                            
                            val_losses = []
                            val_keys = jax.random.split(val_key, len(val_ds))
                            for i, val_batch_np in enumerate(val_ds.as_numpy_iterator()):
                                val_batch_gpu = jax.device_put(val_batch_np)
                                loss = self.validation_loss_fn(master_params_cpu, val_batch_gpu[0], val_batch_gpu[1], self.model.apply, val_keys[i])
                                val_losses.append(loss.item())
                            
                            current_val_loss = np.mean(val_losses) if val_losses else float('inf')
                            # [MODIFIED] Update global HP controller with validation loss
                            self.global_hp_controller.update_q_value(current_val_loss)
                            
                            if current_val_loss < best_val_loss:
                                best_val_loss = current_val_loss
                                if not (active_save_future and not active_save_future.done()):
                                    active_save_future = checkpoint_executor.submit(self._save_cpu_data_task, {'master_params': master_params_cpu, 'val_loss': best_val_loss, 'global_step': global_step}, ckpt_path_best)
                            
                            active_preview_future = preview_executor.submit(_update_preview_task, master_params_cpu, uncond_emb_cpu, self.current_preview_prompt_idx, preview_key)
                    
                    if (global_step + 1) % steps_per_epoch == 0:
                        self.progress.update(epoch_task, advance=1)
                        if not (active_save_future and not active_save_future.done()):
                            # [MODIFIED] Save all Q-Controller states
                            q_states_to_save = {}
                            for i in range(self.num_clones):
                                q_states_to_save[f'lr_controller_{i}'] = self.clone_agents[i]['lr_controller'].state_dict()
                                if 'mmd_controller' in self.clone_agents[i]:
                                    q_states_to_save[f'mmd_controller_{i}'] = self.clone_agents[i]['mmd_controller'].state_dict()
                            q_states_to_save['global_hp_controller'] = self.global_hp_controller.state_dict()
                            
                            master_params_to_save, clone_opt_states_to_save = jax.device_get((p_master_state.params, p_clone_states.opt_state))
                            data_to_save = {'master_params': master_params_to_save, 'clone_opt_states': clone_opt_states_to_save, 'global_step': global_step + 1, **q_states_to_save}
                            active_save_future = checkpoint_executor.submit(self._save_cpu_data_task, data_to_save, ckpt_path_final)
                            self.console.print(f"\n[magenta]Epoch {epoch + 1} complete. Checkpoint saving in background.[/magenta]")
                    
                    current_time = time.time(); self.steps_per_sec = 1.0 / (current_time - last_step_time + 1e-9); last_step_time = time.time()
                    self.progress.update(epoch_task, step=step_in_epoch + 1, best_val_loss=best_val_loss)
                    live.update(self.layout_manager.generate_layout(), refresh=True)
                    global_step += 1
        finally:
            self.console.print(f"\n[yellow]--- Training loop exited. Waiting for background tasks... ---[/yellow]")
            preview_executor.shutdown(wait=True); checkpoint_executor.shutdown(wait=True)
            p_master_state_final, p_stacked_opt_states_final = p_states_for_saving
            master_params_final, clone_opt_states_final = jax.device_get((p_master_state_final.params, p_stacked_opt_states_final.opt_state))
            # [MODIFIED] Save all Q-Controller states
            q_states_to_save = {}
            for i in range(self.num_clones):
                q_states_to_save[f'lr_controller_{i}'] = self.clone_agents[i]['lr_controller'].state_dict()
            ...
            final_data_to_save = {'master_params': master_params_final, 'clone_opt_states': clone_opt_states_final, 'global_step': global_step, **q_states_to_save}
            with open(ckpt_path_final, 'wb') as f: pickle.dump(final_data_to_save, f)
            self.console.print(f"✅ Final resume-state saved to [green]{ckpt_path_final}[/green]")
            config = { 'num_codes': self.args.num_codes, 'num_positions': self.token_map_size, 'd_model': self.args.d_model_cond, 'num_heads': self.args.num_heads, 'num_layers': self.args.num_layers, 'clip_dim': 512 }
            config_path = Path(f"chimera_{self.args.basename}_{self.args.num_layers}l_config.pkl")
            with open(config_path, 'wb') as f: pickle.dump(config, f)
            self.console.print(f"✅ Config saved to [green]{config_path}[/green]")
            if ckpt_path_best.exists(): self.console.print(f"👑 Best model (by validation) remains at [bold magenta]{ckpt_path_best}[/bold magenta]")
...
def main():
    ...
    train_parser.add_argument('--use-bfloat16', action='store_true', help="Use BFloat16 precision for training.")
    ...
    p_chimera = subparsers.add_parser("train-chimera", help="Train the Conductor with the Sentient Chimera architecture.", parents=[base_parser, train_parser])
    p_chimera.add_argument('--data-dir', type=str, required=True)
    p_chimera.add_argument('--latent-grid-size', type=int, required=True)
    p_chimera.add_argument('--epochs', type=int, default=200); p_chimera.add_argument('--batch-size', type=int, default=8)
    p_chimera.add_argument('--lr', type=float, default=1e-4); p_chimera.add_argument('--eval-every', type=int, default=500)
    p_chimera.add_argument('--num-codes', type=int, default=3072); p_chimera.add_argument('--num-layers', type=int, default=12)
    p_chimera.add_argument('--d-model-cond', type=int, default=768); p_chimera.add_argument('--num-heads', type=int, default=8)
    p_chimera.add_argument('--lambda-mmd', type=float, default=0.5); p_chimera.add_argument('--vram-saver-mode', action='store_true')
    p_chimera.add_argument('--mask-ratio', type=float, default=0.5, help="Fraction of tokens to mask during training.")
    ...
    if args.command == "train-chimera":
        if args.d_model_cond % args.num_heads != 0:
            sys.exit(f"[FATAL] Conductor d_model_cond ({args.d_model_cond}) must be divisible by num_heads ({args.num_heads}).")
        if args.latent_grid_size % 4 != 0:
            sys.exit(f"[FATAL] latent_grid_size ({args.latent_grid_size}) must be divisible by 4 due to tokenizer architecture.")
    ...
    elif args.command == "train-chimera": ChimeraConductorTrainer(args).train()
