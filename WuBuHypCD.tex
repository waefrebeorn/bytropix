...
\hypersetup{
    ...
    pdftitle={WuBu Nesting: A Comprehensive Geometric Framework for Adaptive Multi-Scale Hierarchical Representation with Integrated Rotational Dynamics},
    pdfauthor={Your Name(s) Here}, % IMPORTANT: Update this
...
% CORRECTED: Renamed command to avoid conflict
\newcommand{\HypSpace}[3]{\mathcal{H}^{#1}_{#2, #3}} % Hyperbolic space H^n_{c,s}
\newcommand{\tangent}[2]{T_{#1}(\mathcal{H}^{#2})} % Tangent space T_p(H^n)
\newcommand{\R}{\mathbb{R}} % Real numbers symbol
\newcommand{\SO}[1]{\text{SO}(#1)} % SO(n) group
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.58, transform shape, node distance=1.0cm and 1.3cm, auto, >=Latex] % Further reduced scale
        ...
        % Level 1
        \node (proc1) [process, right=2.5cm of tan1] {Level 1 Processing (incl. $F_1$)}; % Positioned right
        ...
        % Transformation 1->2
        \node (rot1) [rotation, right=2.5cm of logmap1] {Rotation $R_1$}; % Positioned right
        ...
        % Level 2 Processing
        \node (proc2) [process, right=2.5cm of tan2] {Level 2 Processing (incl. $F_2$)}; % Positioned right
        ...
        % Transformation 2->3
        \node (rot2) [rotation, right=2.5cm of logmap2] {Rotation $R_2$}; % Positioned right
        ...
        \node (proc3) [process, right=2.5cm of tan3] {Level 3 Processing (incl. $F_3$)}; % Positioned right
