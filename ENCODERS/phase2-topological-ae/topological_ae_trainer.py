...
def main():
    ...
    script_dir = Path(__file__).parent.resolve()
    default_dataset_path = str(script_dir / "fast_dataset")

    parser = argparse.ArgumentParser(description="Topological AE - Scene Decomposer Trainer")
    
    # Updated defaults for your specific setup
    parser.add_argument('--data-dir', type=str, default=default_dataset_path, 
                        help="Path to the directory with SOURCE images.")
    parser.add_argument('--basename', type=str, default="fast_train", 
                        help="Basename for model files.")
    parser.add_argument('--batch-size', type=int, default=1, 
                        help="Batch size PER DEVICE.")
    parser.add_argument('--d-model', type=int, default=64, 
                        help="Core model dimension.")
    parser.add_argument('--latent-dim', type=int, default=64, 
                        help="Final latent feature dimension.")
    
    # Keeping other defaults standard
    parser.add_argument('--latent-grid-size', type=int, default=96)
    ...
    # Automatic safety check: Create the folder if it doesn't exist
    if not Path(args.data_dir).exists():
        print(f"[ERROR] Dataset folder not found at: {args.data_dir}")
        print(f"Please create the folder 'fast_dataset' next to this script and put images in it.")
        sys.exit(1)
